var vidyard_player_width_TCbuWWNS7kVUtiyzoGeUZ5 = 640;
var vidyard_player_height_TCbuWWNS7kVUtiyzoGeUZ5 = 360;
var vidyard_html5_TCbuWWNS7kVUtiyzoGeUZ5 = false;
var vidyard_secure_TCbuWWNS7kVUtiyzoGeUZ5 = false;
var vidyard_integration_check_TCbuWWNS7kVUtiyzoGeUZ5 = function() {};
var vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params = {};
var vidyard_TCbuWWNS7kVUtiyzoGeUZ5_raw_params = '';
...
(function() {
  ...
  var embed_script = document.getElementById('vidyard_embed_code_TCbuWWNS7kVUtiyzoGeUZ5');
  var playerDetails = {"keywords":"Micro800컨트롤러,Micro870컨트롤러,OEM용 마이크로 컨트롤 시스템,PLC,마이크로 애플리케이션,마이크로 컨트롤 시스템,마이크로 컨트롤러,장비 제조업체용 마이크로 컨트롤 시스템,적합한 애플리케이션","thumbnailUrl":"https://play.vidyard.com/TCbuWWNS7kVUtiyzoGeUZ5.jpg","@context":"http://schema.org/","@type":"VideoObject","id":"TCbuWWNS7kVUtiyzoGeUZ5","description":"완전 최신형 Micro870® 컨트롤러는 다양한 애플리케이션에 적합합니다. 자세한 내용은 이 비디오를 참조하십시오! \r\n\r\nMicro870 컨트롤러에 대한 자세한 내용은https://ab.rockwellautomation.com/ko/Programmable-Controllers/Micro870을 참조하십시오.","name":"Micro870® 컨트롤러에 적합한 애플리케이션","transcript":"","uploadDate":"2018-08-07T04:30:18.000Z","duration":"T2M47S","embedUrl":"https://play.vidyard.com/TCbuWWNS7kVUtiyzoGeUZ5","height":360,"width":640};
  ...
  function check_for_duplicate_embeds() {
    ...
    for (var i = 0; i < all_scripts.length; ++i) {
      if (all_scripts[i].getAttribute('id') === "vidyard_embed_code_TCbuWWNS7kVUtiyzoGeUZ5") {
    ...
    if (dupe_scripts.length >= 1) {
      ...
      if (typeof document.querySelectorAll === 'function') {
        ...
        for (var i = 0; i < wrappers.length; ++i) {
          if (wrappers[i].getAttributeNode('onclick').value === "fn_vidyard_TCbuWWNS7kVUtiyzoGeUZ5();") {
  ...
  if (!embed_script || !embed_script.src) {
    ...
    for (var i in scripts) {
      ...
      if (script.src && /^.+(vidyard\.com).*\/TCbuWWNS7kVUtiyzoGeUZ5.*/.test(script.src)) {
        embed_script = script;
        break;
      }
    }
  ...
  if (embed_script && embed_script.src) {
    ...
    vidyard_TCbuWWNS7kVUtiyzoGeUZ5_raw_params = rawQueryParams;
    ...
    embed_script.src.replace(/(?:[\?&])([^&=]*)=?([^&#]*)/g, function ($0, $1, $2) {
      if ($1) {
        switch ($1) {
          ...
          case 'vylegacy':
            vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params[$1] = (parseInt($2, 10) !== 0);
            break;
          case 'chapter':
            vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params[$1] = parseInt($2, 10);
            break;
          case 'width':
          ...
          case 'height':
            ...
            break;
          case 'X-VY-Signature':
            vidyard_secure_TCbuWWNS7kVUtiyzoGeUZ5 = true;
          default:
            vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params[$1] = $2;
            break;
        }
      }
    });
  ...
  if (!vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.type || vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.type !== 'lightbox') {
    vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.type = 'inline';
  }

  // Determine Player Settings which effect embed
  var multi_chapter = false && !(typeof vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.hide_html5_playlist !== 'undefined' && vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.hide_html5_playlist);
  var playlist_always_open = false;
  if (typeof vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.playlist_always_open !== 'undefined') {
    playlist_always_open = vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.playlist_always_open;
  }
  ...
  if (query_player_width && query_player_width > 0 && query_player_height && query_player_height > 0) {
    vidyard_player_width_TCbuWWNS7kVUtiyzoGeUZ5 = query_player_width;
    vidyard_player_height_TCbuWWNS7kVUtiyzoGeUZ5 = query_player_height;
  } else if (query_player_width && query_player_width > 0) {
    query_player_height = (query_player_width * vidyard_player_height_TCbuWWNS7kVUtiyzoGeUZ5) / vidyard_player_width_TCbuWWNS7kVUtiyzoGeUZ5;
    vidyard_player_width_TCbuWWNS7kVUtiyzoGeUZ5 = query_player_width;
    vidyard_player_height_TCbuWWNS7kVUtiyzoGeUZ5 = query_player_height;
  } else if (query_player_height && query_player_height > 0) {
    query_player_width = (query_player_height * vidyard_player_width_TCbuWWNS7kVUtiyzoGeUZ5) / vidyard_player_height_TCbuWWNS7kVUtiyzoGeUZ5;
    vidyard_player_width_TCbuWWNS7kVUtiyzoGeUZ5 = query_player_width;
    vidyard_player_height_TCbuWWNS7kVUtiyzoGeUZ5 = query_player_height;
  }

  var iframe_container = document.getElementById("vidyard_TCbuWWNS7kVUtiyzoGeUZ5");
  ...
  if (!iframe_container && vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.type === 'inline') {
    iframe_container = document.createElement("span");
    iframe_container.id = 'vidyard_TCbuWWNS7kVUtiyzoGeUZ5';
  ...
  if (vidyard_secure_TCbuWWNS7kVUtiyzoGeUZ5 === false) {
    // Get cookie data/ids from various MAPs/CRMs
    var pardot_id = fn_getCookie("pardot");
    if (pardot_id) {
      vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.pardot_id = pardot_id;
    }

    var hubspot_id = fn_getCookie("hubspotutk");
    if (hubspot_id) {
      vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.hubspot_id = hubspot_id;
    }
    ...
    if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
      vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.marketo_id = encodeURIComponent(marketo_id);
    }

    var dreamforce_id = fn_getCookie("vy_dreamforce");
    if (dreamforce_id) {
      vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.dreamforce_id = encodeURIComponent(dreamforce_id);
    }
    ...
    if (vycustom_id !== "") {
      vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.custom_id = vycustom_id;
    }

    // Gets the vyemail query string param, which connects views to an email entered
    var vyemail = get_parameter_by_name('vyemail');
    if (vyemail !== "") {
      vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.email = vyemail;
    }

    var vyac = get_parameter_by_name('vyac');
    var vyplayer = get_parameter_by_name('vyplayer');
    if (vyplayer === 'TCbuWWNS7kVUtiyzoGeUZ5') {
      vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.access_code = vyac;
    }
  }
  ...
  if (navigator.userAgent.toLowerCase().match(/ip(ad|od|hone)|android/)) {
    vidyard_html5_TCbuWWNS7kVUtiyzoGeUZ5 = true;
    vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.disable_popouts = true;
  }
  ...
  function create_embed() {
    ...
    var integration_tracking_check = function() {
      if (playerReady) {
        ...
        window.addEventListener('message', function callback(e) {
          ...
          if (data.event === "ready" && data.uuid === "TCbuWWNS7kVUtiyzoGeUZ5") {
    ...
    var integration_tracking_check_original = function () {
      ...
      if (!referringUrlSent && vidyard_secure_TCbuWWNS7kVUtiyzoGeUZ5 === true && vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.referring_url) {
        var playerFrame = Vidyard.Helpers.getPlayerIFrame('TCbuWWNS7kVUtiyzoGeUZ5');
        var message = {
          'event': 'parentReferer',
          'uuid': 'TCbuWWNS7kVUtiyzoGeUZ5',
          'data': {
            'value': vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.referring_url
          }
        }
      ...
      if (!vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.pardot_id) {
        var pardot_id = fn_getCookie("pardot");
        if (pardot_id) {
          vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.pardot_id = pardot_id;
      ...
      if (!vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.hubspot_id) {
        var hubspot_id = fn_getCookie("hubspotutk");
        if (hubspot_id) {
          vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.hubspot_id = hubspot_id;
      ...
      if (!vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.dreamforce_id) {
        var dreamforce_id = fn_getCookie("vy_dreamforce");
        if (dreamforce_id) {
          dreamforce_id = encodeURIComponent(dreamforce_id);
          vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.dreamforce_id = dreamforce_id;
          messages.push({'data': {'type': 'dreamforce', 'value': dreamforce_id}});
        }
      }
      if (!vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.marketo_id) {
        var marketo_id = fn_getCookie("_mkto_trk");
        var munchkin_id = '';
        
        if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
          marketo_id = encodeURIComponent(marketo_id);
          vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.marketo_id = marketo_id;
      ...
      if (vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.type === "inline" || vidyard_secure_TCbuWWNS7kVUtiyzoGeUZ5 === true) {
        for(var i in messages) {
          messages[i].event = 'associateVisitor';
          messages[i].uuid = 'TCbuWWNS7kVUtiyzoGeUZ5';
          Vidyard.Helpers.getPlayerIFrame('TCbuWWNS7kVUtiyzoGeUZ5').contentWindow.postMessage(JSON.stringify(messages[i]), window.location.protocol + '//play.vidyard.com');
        }
      }
    ...
    vidyard_integration_check_TCbuWWNS7kVUtiyzoGeUZ5 = integration_tracking_check;

    // Display the inline or lightbox
    if (vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.type === "inline") {
      // Load the iframe
      iframe_container.innerHTML = fn_vidyard_build_iframe_TCbuWWNS7kVUtiyzoGeUZ5(false, vidyard_player_width_TCbuWWNS7kVUtiyzoGeUZ5, vidyard_player_height_TCbuWWNS7kVUtiyzoGeUZ5);

      Vidyard.Helpers.addListener(
        'load',
        'onload',
        function() {
          fn_vidyard_iframe_after_load_TCbuWWNS7kVUtiyzoGeUZ5();
          integration_tracking_check();
        },
        Vidyard.Helpers.getPlayerIFrame('TCbuWWNS7kVUtiyzoGeUZ5')
      );

    } else if (vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.type === "lightbox") {
      ...
      if (document.readyState === 'complete') {
        if (!vidyard_secure_TCbuWWNS7kVUtiyzoGeUZ5) {
          integration_tracking_check();
        }
  ...
  Vidyard._players['TCbuWWNS7kVUtiyzoGeUZ5'] = '';
}());
...
var vyContactCallback = function(data) {
  ...
  vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.marketo_lead = marketoData;
  ...
  // or tried again after IFrame loads in fn_vidyard_iframe_after_load_TCbuWWNS7kVUtiyzoGeUZ5
  playerIframe = Vidyard.Helpers.getPlayerIFrame('TCbuWWNS7kVUtiyzoGeUZ5');
  if ( playerIframe ) {
    message = {'data': {'type': 'marketo', 'value': marketoData}};
    message.event = 'identifyVisitor';
    message.uuid = 'TCbuWWNS7kVUtiyzoGeUZ5';
...
(function () {
  function async_load() {
    var contactId = vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.marketo_id;
    if (!contactId) {
    ...
    var url = window.location.protocol + '//play.vidyard.com'
        + '/v3/TCbuWWNS7kVUtiyzoGeUZ5/contact/' + encodeURIComponent(contactId) + '/marketoContact.js';
...
function fn_vidyard_build_iframe_TCbuWWNS7kVUtiyzoGeUZ5(lightbox, iframe_width, iframe_height) {
  if (lightbox) {
    vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.autoplay = true;
  }

  var referring_url = encodeURIComponent(document.referrer);
  if (referring_url) {
    vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.referring_url = referring_url;
  }
  ...
  if (document.documentMode === 7) {
    ...
    var ie7RepaintIframe = function() {
      var iframe = Vidyard.Helpers.getPlayerIFrame('TCbuWWNS7kVUtiyzoGeUZ5');
  ...
  if (vidyard_secure_TCbuWWNS7kVUtiyzoGeUZ5 === true) {
    query_params_string = vidyard_TCbuWWNS7kVUtiyzoGeUZ5_raw_params;
  } else {
    for (var param in vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params) {
      var param_value = vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params[param];
      if (param_value === true) {
        param_value = '1';
      }
  ...
  return '<span id="vidyard_span_TCbuWWNS7kVUtiyzoGeUZ5" style="display: block; margin: auto; position: relative; width: ' + iframe_width + 'px; height: ' + iframe_height + 'px;"><iframe id="vidyard_iframe_TCbuWWNS7kVUtiyzoGeUZ5" class="vidyard_iframe" src="//play.vidyard.com/TCbuWWNS7kVUtiyzoGeUZ5?' + query_params_string + '" width="' + iframe_resizer + '" height="100%" title="Video" aria-label="Video" scrolling=no frameborder=0 allowtransparency="true" allowfullscreen allow="autoplay" style="opacity: 0; background-color: transparent; position: absolute; ' + ios_width_fix + ' right: 0px; top: 0px;"></iframe></span>';
}

// --- If the browser is FF, reload the iframe with the given id
function fn_vidyard_iframe_after_load_TCbuWWNS7kVUtiyzoGeUZ5() {
  var _theframe = Vidyard.Helpers.getPlayerIFrame('TCbuWWNS7kVUtiyzoGeUZ5');
  if (_theframe) {
    ...
    if (vidyard_secure_TCbuWWNS7kVUtiyzoGeUZ5 === true && vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.type === 'lightbox') {
      vidyard_integration_check_TCbuWWNS7kVUtiyzoGeUZ5();
    }
    ...
    if (vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.marketo_lead) {
      var message = {'data': {'type': 'marketo', 'value': vidyard_TCbuWWNS7kVUtiyzoGeUZ5_params.marketo_lead }};
      message.event = 'identifyVisitor';
      message.uuid = 'TCbuWWNS7kVUtiyzoGeUZ5';
      Vidyard.Helpers.getPlayerIFrame('TCbuWWNS7kVUtiyzoGeUZ5')
...
function fn_vidyard_TCbuWWNS7kVUtiyzoGeUZ5() {
  var vidyard = {};
    ...
    return {
      show:function(initial_iframe_settings) {
        var showCore = function () {
          ...
          var width_height = this.lock_to_aspect_ratio(iframe_settings.width, iframe_settings.height, (319 + (vidyard_player_width_TCbuWWNS7kVUtiyzoGeUZ5 / 5)));
      ...
      fill_iframe: function(settings_html_url, has_element, use_post_request, should_animate, width, height) {
        if(has_element) {
          if(iframe_settings.image) {
            ...
            this.set_tinner_size(fn_vidyard_build_iframe_TCbuWWNS7kVUtiyzoGeUZ5(true, width, height), should_animate, width, height);
          } else {
      ...
      resize: function() {
        ...
        var width_height = v_box.lock_to_aspect_ratio(
            vidyard_player_width_TCbuWWNS7kVUtiyzoGeUZ5,
            vidyard_player_height_TCbuWWNS7kVUtiyzoGeUZ5,
            319 + (vidyard_player_width_TCbuWWNS7kVUtiyzoGeUZ5 / 5)
        );

        iframe_settings.width = width_height[0];
        iframe_settings.height = width_height[1];

        var vidyard_iframe = Vidyard.Helpers.getPlayerIFrame('TCbuWWNS7kVUtiyzoGeUZ5');
        var vidyard_span = document.getElementById('vidyard_span_TCbuWWNS7kVUtiyzoGeUZ5');
      ...
      fade_element: function(element_to_fade, fading_in, target_alpha) {
        ...
        if (old_opacity == target_alpha) {
          ...
          if (fading_in == -1) {
            ...
            Vidyard.Helpers.getPlayerIFrame('TCbuWWNS7kVUtiyzoGeUZ5').src = '';
  ...
  vidyard.box.show({
    iframe: '#', width: vidyard_player_width_TCbuWWNS7kVUtiyzoGeUZ5,
    height: vidyard_player_height_TCbuWWNS7kVUtiyzoGeUZ5,
    animate: !vidyard_html5_TCbuWWNS7kVUtiyzoGeUZ5,
    fixed: true
  });
}
