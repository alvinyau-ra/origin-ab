var vidyard_player_width_XXEQQ3aMPTRLcXSXJNbSHu = 640;
var vidyard_player_height_XXEQQ3aMPTRLcXSXJNbSHu = 360;
var vidyard_html5_XXEQQ3aMPTRLcXSXJNbSHu = false;
var vidyard_secure_XXEQQ3aMPTRLcXSXJNbSHu = false;
var vidyard_integration_check_XXEQQ3aMPTRLcXSXJNbSHu = function() {};
var vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params = {};
var vidyard_XXEQQ3aMPTRLcXSXJNbSHu_raw_params = '';
...
(function() {
  ...
  var embed_script = document.getElementById('vidyard_embed_code_XXEQQ3aMPTRLcXSXJNbSHu');
  var playerDetails = {"keywords":"IHM,PanelView 5000,Terminal graphique,intégration Logix,video","thumbnailUrl":"https://play.vidyard.com/XXEQQ3aMPTRLcXSXJNbSHu.jpg","@context":"http://schema.org/","@type":"VideoObject","id":"XXEQQ3aMPTRLcXSXJNbSHu","description":"Nos terminaux graphiques PanelView™ 5000 offrent un design moderne et intuitif et permettent une intégration Logix optimisée qui utilise le logiciel Studio 5000 View Designer™. Cliquez ici pour en savoir plus: http://ab.rockwellautomation.com/Graphic-Terminals/2715-PanelView-5500-Graphic-Terminals","name":"Vidéo de présentation de la gamme de terminaux graphiques PanelView 5000","transcript":"","uploadDate":"2018-09-04T17:29:25.000Z","duration":"T2M45S","embedUrl":"https://play.vidyard.com/XXEQQ3aMPTRLcXSXJNbSHu","height":360,"width":640};
  ...
  function check_for_duplicate_embeds() {
    ...
    for (var i = 0; i < all_scripts.length; ++i) {
      if (all_scripts[i].getAttribute('id') === "vidyard_embed_code_XXEQQ3aMPTRLcXSXJNbSHu") {
    ...
    if (dupe_scripts.length >= 1) {
      ...
      if (typeof document.querySelectorAll === 'function') {
        ...
        for (var i = 0; i < wrappers.length; ++i) {
          if (wrappers[i].getAttributeNode('onclick').value === "fn_vidyard_XXEQQ3aMPTRLcXSXJNbSHu();") {
  ...
  if (!embed_script || !embed_script.src) {
    ...
    for (var i in scripts) {
      ...
      if (script.src && /^.+(vidyard\.com).*\/XXEQQ3aMPTRLcXSXJNbSHu.*/.test(script.src)) {
        embed_script = script;
        break;
      }
    }
  ...
  if (embed_script && embed_script.src) {
    ...
    vidyard_XXEQQ3aMPTRLcXSXJNbSHu_raw_params = rawQueryParams;
    ...
    embed_script.src.replace(/(?:[\?&])([^&=]*)=?([^&#]*)/g, function ($0, $1, $2) {
      if ($1) {
        switch ($1) {
          ...
          case 'vylegacy':
            vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params[$1] = (parseInt($2, 10) !== 0);
            break;
          case 'chapter':
            vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params[$1] = parseInt($2, 10);
            break;
          case 'width':
          ...
          case 'height':
            ...
            break;
          case 'X-VY-Signature':
            vidyard_secure_XXEQQ3aMPTRLcXSXJNbSHu = true;
          default:
            vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params[$1] = $2;
            break;
        }
      }
    });
  ...
  if (!vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.type || vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.type !== 'lightbox') {
    vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.type = 'inline';
  }

  // Determine Player Settings which effect embed
  var multi_chapter = false && !(typeof vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.hide_html5_playlist !== 'undefined' && vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.hide_html5_playlist);
  var playlist_always_open = false;
  if (typeof vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.playlist_always_open !== 'undefined') {
    playlist_always_open = vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.playlist_always_open;
  }
  ...
  if (query_player_width && query_player_width > 0 && query_player_height && query_player_height > 0) {
    vidyard_player_width_XXEQQ3aMPTRLcXSXJNbSHu = query_player_width;
    vidyard_player_height_XXEQQ3aMPTRLcXSXJNbSHu = query_player_height;
  } else if (query_player_width && query_player_width > 0) {
    query_player_height = (query_player_width * vidyard_player_height_XXEQQ3aMPTRLcXSXJNbSHu) / vidyard_player_width_XXEQQ3aMPTRLcXSXJNbSHu;
    vidyard_player_width_XXEQQ3aMPTRLcXSXJNbSHu = query_player_width;
    vidyard_player_height_XXEQQ3aMPTRLcXSXJNbSHu = query_player_height;
  } else if (query_player_height && query_player_height > 0) {
    query_player_width = (query_player_height * vidyard_player_width_XXEQQ3aMPTRLcXSXJNbSHu) / vidyard_player_height_XXEQQ3aMPTRLcXSXJNbSHu;
    vidyard_player_width_XXEQQ3aMPTRLcXSXJNbSHu = query_player_width;
    vidyard_player_height_XXEQQ3aMPTRLcXSXJNbSHu = query_player_height;
  }

  var iframe_container = document.getElementById("vidyard_XXEQQ3aMPTRLcXSXJNbSHu");
  ...
  if (!iframe_container && vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.type === 'inline') {
    iframe_container = document.createElement("span");
    iframe_container.id = 'vidyard_XXEQQ3aMPTRLcXSXJNbSHu';
  ...
  if (vidyard_secure_XXEQQ3aMPTRLcXSXJNbSHu === false) {
    // Get cookie data/ids from various MAPs/CRMs
    var pardot_id = fn_getCookie("pardot");
    if (pardot_id) {
      vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.pardot_id = pardot_id;
    }

    var hubspot_id = fn_getCookie("hubspotutk");
    if (hubspot_id) {
      vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.hubspot_id = hubspot_id;
    }
    ...
    if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
      vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.marketo_id = encodeURIComponent(marketo_id);
    }

    var dreamforce_id = fn_getCookie("vy_dreamforce");
    if (dreamforce_id) {
      vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.dreamforce_id = encodeURIComponent(dreamforce_id);
    }
    ...
    if (vycustom_id !== "") {
      vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.custom_id = vycustom_id;
    }

    // Gets the vyemail query string param, which connects views to an email entered
    var vyemail = get_parameter_by_name('vyemail');
    if (vyemail !== "") {
      vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.email = vyemail;
    }

    var vyac = get_parameter_by_name('vyac');
    var vyplayer = get_parameter_by_name('vyplayer');
    if (vyplayer === 'XXEQQ3aMPTRLcXSXJNbSHu') {
      vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.access_code = vyac;
    }
  }
  ...
  if (navigator.userAgent.toLowerCase().match(/ip(ad|od|hone)|android/)) {
    vidyard_html5_XXEQQ3aMPTRLcXSXJNbSHu = true;
    vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.disable_popouts = true;
  }
  ...
  function create_embed() {
    ...
    var integration_tracking_check = function() {
      if (playerReady) {
        ...
        window.addEventListener('message', function callback(e) {
          ...
          if (data.event === "ready" && data.uuid === "XXEQQ3aMPTRLcXSXJNbSHu") {
    ...
    var integration_tracking_check_original = function () {
      ...
      if (!referringUrlSent && vidyard_secure_XXEQQ3aMPTRLcXSXJNbSHu === true && vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.referring_url) {
        var playerFrame = Vidyard.Helpers.getPlayerIFrame('XXEQQ3aMPTRLcXSXJNbSHu');
        var message = {
          'event': 'parentReferer',
          'uuid': 'XXEQQ3aMPTRLcXSXJNbSHu',
          'data': {
            'value': vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.referring_url
          }
        }
      ...
      if (!vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.pardot_id) {
        var pardot_id = fn_getCookie("pardot");
        if (pardot_id) {
          vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.pardot_id = pardot_id;
      ...
      if (!vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.hubspot_id) {
        var hubspot_id = fn_getCookie("hubspotutk");
        if (hubspot_id) {
          vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.hubspot_id = hubspot_id;
      ...
      if (!vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.dreamforce_id) {
        var dreamforce_id = fn_getCookie("vy_dreamforce");
        if (dreamforce_id) {
          dreamforce_id = encodeURIComponent(dreamforce_id);
          vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.dreamforce_id = dreamforce_id;
          messages.push({'data': {'type': 'dreamforce', 'value': dreamforce_id}});
        }
      }
      if (!vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.marketo_id) {
        var marketo_id = fn_getCookie("_mkto_trk");
        var munchkin_id = '';
        
        if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
          marketo_id = encodeURIComponent(marketo_id);
          vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.marketo_id = marketo_id;
        ...
        if (!eloquaSent &&
            vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.eloqua_id &&
            ((vidyard_secure_XXEQQ3aMPTRLcXSXJNbSHu === true && vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.type === 'lightbox') ||
             (vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.type === 'inline'))) {
          eloquaSent = true;
          messages.push({'data': {'type': 'eloqua', 'value': vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.eloqua_id}});
        }
      

      // inline embeds are ready to receive api requests
      // lightbox will pass data through querystring params
      if (vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.type === "inline" || vidyard_secure_XXEQQ3aMPTRLcXSXJNbSHu === true) {
        for(var i in messages) {
          messages[i].event = 'associateVisitor';
          messages[i].uuid = 'XXEQQ3aMPTRLcXSXJNbSHu';
          Vidyard.Helpers.getPlayerIFrame('XXEQQ3aMPTRLcXSXJNbSHu').contentWindow.postMessage(JSON.stringify(messages[i]), window.location.protocol + '//play.vidyard.com');
        }
      }
    ...
    vidyard_integration_check_XXEQQ3aMPTRLcXSXJNbSHu = integration_tracking_check;

    // Display the inline or lightbox
    if (vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.type === "inline") {
      // Load the iframe
      iframe_container.innerHTML = fn_vidyard_build_iframe_XXEQQ3aMPTRLcXSXJNbSHu(false, vidyard_player_width_XXEQQ3aMPTRLcXSXJNbSHu, vidyard_player_height_XXEQQ3aMPTRLcXSXJNbSHu);

      Vidyard.Helpers.addListener(
        'load',
        'onload',
        function() {
          fn_vidyard_iframe_after_load_XXEQQ3aMPTRLcXSXJNbSHu();
          integration_tracking_check();
        },
        Vidyard.Helpers.getPlayerIFrame('XXEQQ3aMPTRLcXSXJNbSHu')
      );

    } else if (vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.type === "lightbox") {
      ...
      if (document.readyState === 'complete') {
        if (!vidyard_secure_XXEQQ3aMPTRLcXSXJNbSHu) {
          integration_tracking_check();
        }
    ...
    function getEloquaCookie() {
      if (timerId !== null) {
        ...
        if (typeof this.GetElqCustomerGUID === 'function') {
          ...
          vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.eloqua_id = eloqua_id;
          return;
        } else if (this._elqQ) {
  ...
  Vidyard._players['XXEQQ3aMPTRLcXSXJNbSHu'] = '';
}());
...
var vyContactCallback = function(data) {
  ...
  vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.marketo_lead = marketoData;
  ...
  // or tried again after IFrame loads in fn_vidyard_iframe_after_load_XXEQQ3aMPTRLcXSXJNbSHu
  playerIframe = Vidyard.Helpers.getPlayerIFrame('XXEQQ3aMPTRLcXSXJNbSHu');
  if ( playerIframe ) {
    message = {'data': {'type': 'marketo', 'value': marketoData}};
    message.event = 'identifyVisitor';
    message.uuid = 'XXEQQ3aMPTRLcXSXJNbSHu';
...
(function () {
  function async_load() {
    var contactId = vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.marketo_id;
    if (!contactId) {
    ...
    var url = window.location.protocol + '//play.vidyard.com'
        + '/v3/XXEQQ3aMPTRLcXSXJNbSHu/contact/' + encodeURIComponent(contactId) + '/marketoContact.js';
...
function fn_vidyard_build_iframe_XXEQQ3aMPTRLcXSXJNbSHu(lightbox, iframe_width, iframe_height) {
  if (lightbox) {
    vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.autoplay = true;
  }

  var referring_url = encodeURIComponent(document.referrer);
  if (referring_url) {
    vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.referring_url = referring_url;
  }
  ...
  if (document.documentMode === 7) {
    ...
    var ie7RepaintIframe = function() {
      var iframe = Vidyard.Helpers.getPlayerIFrame('XXEQQ3aMPTRLcXSXJNbSHu');
  ...
  if (vidyard_secure_XXEQQ3aMPTRLcXSXJNbSHu === true) {
    query_params_string = vidyard_XXEQQ3aMPTRLcXSXJNbSHu_raw_params;
  } else {
    for (var param in vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params) {
      var param_value = vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params[param];
      if (param_value === true) {
        param_value = '1';
      }
...
function fn_vidyard_iframe_after_load_XXEQQ3aMPTRLcXSXJNbSHu() {
  ...
  if (_theframe) {
    ...
    if (vidyard_secure_XXEQQ3aMPTRLcXSXJNbSHu === true && vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.type === 'lightbox') {
      vidyard_integration_check_XXEQQ3aMPTRLcXSXJNbSHu();
    }
    ...
    if (vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.marketo_lead) {
      var message = {'data': {'type': 'marketo', 'value': vidyard_XXEQQ3aMPTRLcXSXJNbSHu_params.marketo_lead }};
      message.event = 'identifyVisitor';
      message.uuid = 'XXEQQ3aMPTRLcXSXJNbSHu';
      Vidyard.Helpers.getPlayerIFrame('XXEQQ3aMPTRLcXSXJNbSHu')
...
function fn_vidyard_XXEQQ3aMPTRLcXSXJNbSHu() {
  var vidyard = {};
    ...
    return {
      show:function(initial_iframe_settings) {
        var showCore = function () {
          ...
          var width_height = this.lock_to_aspect_ratio(iframe_settings.width, iframe_settings.height, (319 + (vidyard_player_width_XXEQQ3aMPTRLcXSXJNbSHu / 5)));
      ...
      fill_iframe: function(settings_html_url, has_element, use_post_request, should_animate, width, height) {
        if(has_element) {
          if(iframe_settings.image) {
            ...
            this.set_tinner_size(fn_vidyard_build_iframe_XXEQQ3aMPTRLcXSXJNbSHu(true, width, height), should_animate, width, height);
          } else {
      ...
      resize: function() {
        ...
        var width_height = v_box.lock_to_aspect_ratio(
            vidyard_player_width_XXEQQ3aMPTRLcXSXJNbSHu,
            vidyard_player_height_XXEQQ3aMPTRLcXSXJNbSHu,
            319 + (vidyard_player_width_XXEQQ3aMPTRLcXSXJNbSHu / 5)
        );

        iframe_settings.width = width_height[0];
        iframe_settings.height = width_height[1];

        var vidyard_iframe = Vidyard.Helpers.getPlayerIFrame('XXEQQ3aMPTRLcXSXJNbSHu');
        var vidyard_span = document.getElementById('vidyard_span_XXEQQ3aMPTRLcXSXJNbSHu');
      ...
      fade_element: function(element_to_fade, fading_in, target_alpha) {
        ...
        if (old_opacity == target_alpha) {
          ...
          if (fading_in == -1) {
            ...
            Vidyard.Helpers.getPlayerIFrame('XXEQQ3aMPTRLcXSXJNbSHu').src = '';
  ...
  vidyard.box.show({
    iframe: '#', width: vidyard_player_width_XXEQQ3aMPTRLcXSXJNbSHu,
    height: vidyard_player_height_XXEQQ3aMPTRLcXSXJNbSHu,
    animate: !vidyard_html5_XXEQQ3aMPTRLcXSXJNbSHu,
    fixed: true
  });
}
