var vidyard_player_width_vEpVRTLna1obX4gtcEMVzp = 640;
var vidyard_player_height_vEpVRTLna1obX4gtcEMVzp = 360;
var vidyard_html5_vEpVRTLna1obX4gtcEMVzp = false;
var vidyard_secure_vEpVRTLna1obX4gtcEMVzp = false;
var vidyard_integration_check_vEpVRTLna1obX4gtcEMVzp = function() {};
var vidyard_vEpVRTLna1obX4gtcEMVzp_params = {};
var vidyard_vEpVRTLna1obX4gtcEMVzp_raw_params = '';
...
(function() {
  ...
  var embed_script = document.getElementById('vidyard_embed_code_vEpVRTLna1obX4gtcEMVzp');
  var playerDetails = {"keywords":"A maior classifi,Aço inoxidável,Baseado em microprocessadores,Estado sólido,Parada de emergência","thumbnailUrl":"https://play.vidyard.com/vEpVRTLna1obX4gtcEMVzp.jpg","@context":"http://schema.org/","@type":"VideoObject","id":"vEpVRTLna1obX4gtcEMVzp","description":"A capacidade de diagnóstico da chave de corda Guardmaster Lifeline™ 5 lhe permite enfrentar possíveis problemas antes que gerem uma parada não programada. https://ab.rockwellautomation.com/pt/push-buttons/emergency-stop/cable-pull/lifeline-5","name":"Vídeo da Chave de Corda Guardmaster® Lifeline 5","transcript":"","uploadDate":"2018-01-17T20:09:28.000Z","duration":"T2M25S","embedUrl":"https://play.vidyard.com/vEpVRTLna1obX4gtcEMVzp","height":360,"width":640};
  ...
  function check_for_duplicate_embeds() {
    ...
    for (var i = 0; i < all_scripts.length; ++i) {
      if (all_scripts[i].getAttribute('id') === "vidyard_embed_code_vEpVRTLna1obX4gtcEMVzp") {
    ...
    if (dupe_scripts.length >= 1) {
      ...
      if (typeof document.querySelectorAll === 'function') {
        ...
        for (var i = 0; i < wrappers.length; ++i) {
          if (wrappers[i].getAttributeNode('onclick').value === "fn_vidyard_vEpVRTLna1obX4gtcEMVzp();") {
  ...
  if (!embed_script || !embed_script.src) {
    ...
    for (var i in scripts) {
      ...
      if (script.src && /^.+(vidyard\.com).*\/vEpVRTLna1obX4gtcEMVzp.*/.test(script.src)) {
        embed_script = script;
        break;
      }
    }
  ...
  if (embed_script && embed_script.src) {
    ...
    vidyard_vEpVRTLna1obX4gtcEMVzp_raw_params = rawQueryParams;
    ...
    embed_script.src.replace(/(?:[\?&])([^&=]*)=?([^&#]*)/g, function ($0, $1, $2) {
      if ($1) {
        switch ($1) {
          ...
          case 'vylegacy':
            vidyard_vEpVRTLna1obX4gtcEMVzp_params[$1] = (parseInt($2, 10) !== 0);
            break;
          case 'chapter':
            vidyard_vEpVRTLna1obX4gtcEMVzp_params[$1] = parseInt($2, 10);
            break;
          case 'width':
          ...
          case 'height':
            ...
            break;
          case 'X-VY-Signature':
            vidyard_secure_vEpVRTLna1obX4gtcEMVzp = true;
          default:
            vidyard_vEpVRTLna1obX4gtcEMVzp_params[$1] = $2;
            break;
        }
      }
    });
  ...
  if (!vidyard_vEpVRTLna1obX4gtcEMVzp_params.type || vidyard_vEpVRTLna1obX4gtcEMVzp_params.type !== 'lightbox') {
    vidyard_vEpVRTLna1obX4gtcEMVzp_params.type = 'inline';
  }

  // Determine Player Settings which effect embed
  var multi_chapter = false && !(typeof vidyard_vEpVRTLna1obX4gtcEMVzp_params.hide_html5_playlist !== 'undefined' && vidyard_vEpVRTLna1obX4gtcEMVzp_params.hide_html5_playlist);
  var playlist_always_open = false;
  if (typeof vidyard_vEpVRTLna1obX4gtcEMVzp_params.playlist_always_open !== 'undefined') {
    playlist_always_open = vidyard_vEpVRTLna1obX4gtcEMVzp_params.playlist_always_open;
  }
  ...
  if (query_player_width && query_player_width > 0 && query_player_height && query_player_height > 0) {
    vidyard_player_width_vEpVRTLna1obX4gtcEMVzp = query_player_width;
    vidyard_player_height_vEpVRTLna1obX4gtcEMVzp = query_player_height;
  } else if (query_player_width && query_player_width > 0) {
    query_player_height = (query_player_width * vidyard_player_height_vEpVRTLna1obX4gtcEMVzp) / vidyard_player_width_vEpVRTLna1obX4gtcEMVzp;
    vidyard_player_width_vEpVRTLna1obX4gtcEMVzp = query_player_width;
    vidyard_player_height_vEpVRTLna1obX4gtcEMVzp = query_player_height;
  } else if (query_player_height && query_player_height > 0) {
    query_player_width = (query_player_height * vidyard_player_width_vEpVRTLna1obX4gtcEMVzp) / vidyard_player_height_vEpVRTLna1obX4gtcEMVzp;
    vidyard_player_width_vEpVRTLna1obX4gtcEMVzp = query_player_width;
    vidyard_player_height_vEpVRTLna1obX4gtcEMVzp = query_player_height;
  }

  var iframe_container = document.getElementById("vidyard_vEpVRTLna1obX4gtcEMVzp");
  ...
  if (!iframe_container && vidyard_vEpVRTLna1obX4gtcEMVzp_params.type === 'inline') {
    iframe_container = document.createElement("span");
    iframe_container.id = 'vidyard_vEpVRTLna1obX4gtcEMVzp';
  ...
  if (vidyard_secure_vEpVRTLna1obX4gtcEMVzp === false) {
    // Get cookie data/ids from various MAPs/CRMs
    var pardot_id = fn_getCookie("pardot");
    if (pardot_id) {
      vidyard_vEpVRTLna1obX4gtcEMVzp_params.pardot_id = pardot_id;
    }

    var hubspot_id = fn_getCookie("hubspotutk");
    if (hubspot_id) {
      vidyard_vEpVRTLna1obX4gtcEMVzp_params.hubspot_id = hubspot_id;
    }
    ...
    if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
      vidyard_vEpVRTLna1obX4gtcEMVzp_params.marketo_id = encodeURIComponent(marketo_id);
    }

    var dreamforce_id = fn_getCookie("vy_dreamforce");
    if (dreamforce_id) {
      vidyard_vEpVRTLna1obX4gtcEMVzp_params.dreamforce_id = encodeURIComponent(dreamforce_id);
    }
    ...
    if (vycustom_id !== "") {
      vidyard_vEpVRTLna1obX4gtcEMVzp_params.custom_id = vycustom_id;
    }

    // Gets the vyemail query string param, which connects views to an email entered
    var vyemail = get_parameter_by_name('vyemail');
    if (vyemail !== "") {
      vidyard_vEpVRTLna1obX4gtcEMVzp_params.email = vyemail;
    }

    var vyac = get_parameter_by_name('vyac');
    var vyplayer = get_parameter_by_name('vyplayer');
    if (vyplayer === 'vEpVRTLna1obX4gtcEMVzp') {
      vidyard_vEpVRTLna1obX4gtcEMVzp_params.access_code = vyac;
    }
  }
  ...
  if (navigator.userAgent.toLowerCase().match(/ip(ad|od|hone)|android/)) {
    vidyard_html5_vEpVRTLna1obX4gtcEMVzp = true;
    vidyard_vEpVRTLna1obX4gtcEMVzp_params.disable_popouts = true;
  }
  ...
  function create_embed() {
    ...
    var integration_tracking_check = function() {
      if (playerReady) {
        ...
        window.addEventListener('message', function callback(e) {
          ...
          if (data.event === "ready" && data.uuid === "vEpVRTLna1obX4gtcEMVzp") {
    ...
    var integration_tracking_check_original = function () {
      ...
      if (!referringUrlSent && vidyard_secure_vEpVRTLna1obX4gtcEMVzp === true && vidyard_vEpVRTLna1obX4gtcEMVzp_params.referring_url) {
        var playerFrame = Vidyard.Helpers.getPlayerIFrame('vEpVRTLna1obX4gtcEMVzp');
        var message = {
          'event': 'parentReferer',
          'uuid': 'vEpVRTLna1obX4gtcEMVzp',
          'data': {
            'value': vidyard_vEpVRTLna1obX4gtcEMVzp_params.referring_url
          }
        }
      ...
      if (!vidyard_vEpVRTLna1obX4gtcEMVzp_params.pardot_id) {
        var pardot_id = fn_getCookie("pardot");
        if (pardot_id) {
          vidyard_vEpVRTLna1obX4gtcEMVzp_params.pardot_id = pardot_id;
      ...
      if (!vidyard_vEpVRTLna1obX4gtcEMVzp_params.hubspot_id) {
        var hubspot_id = fn_getCookie("hubspotutk");
        if (hubspot_id) {
          vidyard_vEpVRTLna1obX4gtcEMVzp_params.hubspot_id = hubspot_id;
      ...
      if (!vidyard_vEpVRTLna1obX4gtcEMVzp_params.dreamforce_id) {
        var dreamforce_id = fn_getCookie("vy_dreamforce");
        if (dreamforce_id) {
          dreamforce_id = encodeURIComponent(dreamforce_id);
          vidyard_vEpVRTLna1obX4gtcEMVzp_params.dreamforce_id = dreamforce_id;
          messages.push({'data': {'type': 'dreamforce', 'value': dreamforce_id}});
        }
      }
      if (!vidyard_vEpVRTLna1obX4gtcEMVzp_params.marketo_id) {
        var marketo_id = fn_getCookie("_mkto_trk");
        var munchkin_id = '';
        
        if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
          marketo_id = encodeURIComponent(marketo_id);
          vidyard_vEpVRTLna1obX4gtcEMVzp_params.marketo_id = marketo_id;
        ...
        if (!eloquaSent &&
            vidyard_vEpVRTLna1obX4gtcEMVzp_params.eloqua_id &&
            ((vidyard_secure_vEpVRTLna1obX4gtcEMVzp === true && vidyard_vEpVRTLna1obX4gtcEMVzp_params.type === 'lightbox') ||
             (vidyard_vEpVRTLna1obX4gtcEMVzp_params.type === 'inline'))) {
          eloquaSent = true;
          messages.push({'data': {'type': 'eloqua', 'value': vidyard_vEpVRTLna1obX4gtcEMVzp_params.eloqua_id}});
        }
      

      // inline embeds are ready to receive api requests
      // lightbox will pass data through querystring params
      if (vidyard_vEpVRTLna1obX4gtcEMVzp_params.type === "inline" || vidyard_secure_vEpVRTLna1obX4gtcEMVzp === true) {
        for(var i in messages) {
          messages[i].event = 'associateVisitor';
          messages[i].uuid = 'vEpVRTLna1obX4gtcEMVzp';
          Vidyard.Helpers.getPlayerIFrame('vEpVRTLna1obX4gtcEMVzp').contentWindow.postMessage(JSON.stringify(messages[i]), window.location.protocol + '//play.vidyard.com');
        }
      }
    ...
    vidyard_integration_check_vEpVRTLna1obX4gtcEMVzp = integration_tracking_check;

    // Display the inline or lightbox
    if (vidyard_vEpVRTLna1obX4gtcEMVzp_params.type === "inline") {
      // Load the iframe
      iframe_container.innerHTML = fn_vidyard_build_iframe_vEpVRTLna1obX4gtcEMVzp(false, vidyard_player_width_vEpVRTLna1obX4gtcEMVzp, vidyard_player_height_vEpVRTLna1obX4gtcEMVzp);

      Vidyard.Helpers.addListener(
        'load',
        'onload',
        function() {
          fn_vidyard_iframe_after_load_vEpVRTLna1obX4gtcEMVzp();
          integration_tracking_check();
        },
        Vidyard.Helpers.getPlayerIFrame('vEpVRTLna1obX4gtcEMVzp')
      );

    } else if (vidyard_vEpVRTLna1obX4gtcEMVzp_params.type === "lightbox") {
      ...
      if (document.readyState === 'complete') {
        if (!vidyard_secure_vEpVRTLna1obX4gtcEMVzp) {
          integration_tracking_check();
        }
    ...
    function getEloquaCookie() {
      if (timerId !== null) {
        ...
        if (typeof this.GetElqCustomerGUID === 'function') {
          ...
          vidyard_vEpVRTLna1obX4gtcEMVzp_params.eloqua_id = eloqua_id;
          return;
        } else if (this._elqQ) {
  ...
  Vidyard._players['vEpVRTLna1obX4gtcEMVzp'] = '';
}());
...
var vyContactCallback = function(data) {
  ...
  vidyard_vEpVRTLna1obX4gtcEMVzp_params.marketo_lead = marketoData;
  ...
  // or tried again after IFrame loads in fn_vidyard_iframe_after_load_vEpVRTLna1obX4gtcEMVzp
  playerIframe = Vidyard.Helpers.getPlayerIFrame('vEpVRTLna1obX4gtcEMVzp');
  if ( playerIframe ) {
    message = {'data': {'type': 'marketo', 'value': marketoData}};
    message.event = 'identifyVisitor';
    message.uuid = 'vEpVRTLna1obX4gtcEMVzp';
...
(function () {
  function async_load() {
    var contactId = vidyard_vEpVRTLna1obX4gtcEMVzp_params.marketo_id;
    if (!contactId) {
    ...
    var url = window.location.protocol + '//play.vidyard.com'
        + '/v3/vEpVRTLna1obX4gtcEMVzp/contact/' + encodeURIComponent(contactId) + '/marketoContact.js';
...
function fn_vidyard_build_iframe_vEpVRTLna1obX4gtcEMVzp(lightbox, iframe_width, iframe_height) {
  if (lightbox) {
    vidyard_vEpVRTLna1obX4gtcEMVzp_params.autoplay = true;
  }

  var referring_url = encodeURIComponent(document.referrer);
  if (referring_url) {
    vidyard_vEpVRTLna1obX4gtcEMVzp_params.referring_url = referring_url;
  }
  ...
  if (document.documentMode === 7) {
    ...
    var ie7RepaintIframe = function() {
      var iframe = Vidyard.Helpers.getPlayerIFrame('vEpVRTLna1obX4gtcEMVzp');
  ...
  if (vidyard_secure_vEpVRTLna1obX4gtcEMVzp === true) {
    query_params_string = vidyard_vEpVRTLna1obX4gtcEMVzp_raw_params;
  } else {
    for (var param in vidyard_vEpVRTLna1obX4gtcEMVzp_params) {
      var param_value = vidyard_vEpVRTLna1obX4gtcEMVzp_params[param];
      if (param_value === true) {
        param_value = '1';
      }
...
function fn_vidyard_iframe_after_load_vEpVRTLna1obX4gtcEMVzp() {
  ...
  if (_theframe) {
    ...
    if (vidyard_secure_vEpVRTLna1obX4gtcEMVzp === true && vidyard_vEpVRTLna1obX4gtcEMVzp_params.type === 'lightbox') {
      vidyard_integration_check_vEpVRTLna1obX4gtcEMVzp();
    }
    ...
    if (vidyard_vEpVRTLna1obX4gtcEMVzp_params.marketo_lead) {
      var message = {'data': {'type': 'marketo', 'value': vidyard_vEpVRTLna1obX4gtcEMVzp_params.marketo_lead }};
      message.event = 'identifyVisitor';
      message.uuid = 'vEpVRTLna1obX4gtcEMVzp';
      Vidyard.Helpers.getPlayerIFrame('vEpVRTLna1obX4gtcEMVzp')
...
function fn_vidyard_vEpVRTLna1obX4gtcEMVzp() {
  var vidyard = {};
    ...
    return {
      show:function(initial_iframe_settings) {
        var showCore = function () {
          ...
          var width_height = this.lock_to_aspect_ratio(iframe_settings.width, iframe_settings.height, (319 + (vidyard_player_width_vEpVRTLna1obX4gtcEMVzp / 5)));
      ...
      fill_iframe: function(settings_html_url, has_element, use_post_request, should_animate, width, height) {
        if(has_element) {
          if(iframe_settings.image) {
            ...
            this.set_tinner_size(fn_vidyard_build_iframe_vEpVRTLna1obX4gtcEMVzp(true, width, height), should_animate, width, height);
          } else {
      ...
      resize: function() {
        ...
        var width_height = v_box.lock_to_aspect_ratio(
            vidyard_player_width_vEpVRTLna1obX4gtcEMVzp,
            vidyard_player_height_vEpVRTLna1obX4gtcEMVzp,
            319 + (vidyard_player_width_vEpVRTLna1obX4gtcEMVzp / 5)
        );

        iframe_settings.width = width_height[0];
        iframe_settings.height = width_height[1];

        var vidyard_iframe = Vidyard.Helpers.getPlayerIFrame('vEpVRTLna1obX4gtcEMVzp');
        var vidyard_span = document.getElementById('vidyard_span_vEpVRTLna1obX4gtcEMVzp');
      ...
      fade_element: function(element_to_fade, fading_in, target_alpha) {
        ...
        if (old_opacity == target_alpha) {
          ...
          if (fading_in == -1) {
            ...
            Vidyard.Helpers.getPlayerIFrame('vEpVRTLna1obX4gtcEMVzp').src = '';
  ...
  vidyard.box.show({
    iframe: '#', width: vidyard_player_width_vEpVRTLna1obX4gtcEMVzp,
    height: vidyard_player_height_vEpVRTLna1obX4gtcEMVzp,
    animate: !vidyard_html5_vEpVRTLna1obX4gtcEMVzp,
    fixed: true
  });
}
