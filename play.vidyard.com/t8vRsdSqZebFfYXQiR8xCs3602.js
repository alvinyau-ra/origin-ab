var vidyard_player_width_t8vRsdSqZebFfYXQiR8xCs = 640;
var vidyard_player_height_t8vRsdSqZebFfYXQiR8xCs = 360;
var vidyard_html5_t8vRsdSqZebFfYXQiR8xCs = false;
var vidyard_secure_t8vRsdSqZebFfYXQiR8xCs = false;
var vidyard_integration_check_t8vRsdSqZebFfYXQiR8xCs = function() {};
var vidyard_t8vRsdSqZebFfYXQiR8xCs_params = {};
var vidyard_t8vRsdSqZebFfYXQiR8xCs_raw_params = '';
...
(function() {
  ...
  var embed_script = document.getElementById('vidyard_embed_code_t8vRsdSqZebFfYXQiR8xCs');
  var playerDetails = {"keywords":"FLEX 5000 I/O,FLEX I/O,I/O 모듈,In-cabinet 모듈형,Logix 5000 시리즈,분산 I/O 솔루션,안전 I/O 모듈","thumbnailUrl":"https://play.vidyard.com/t8vRsdSqZebFfYXQiR8xCs.jpg","@context":"http://schema.org/","@type":"VideoObject","id":"t8vRsdSqZebFfYXQiR8xCs","description":"클린룸 제조에서부터 척박한 환경에 이르기까지 고객의 애플리케이션 요구 사항에 적합하도록 강화된 새로운 분산 I/O 솔루션인 FLEX 5000 I/O에 대해 알아보십시오. 유연하고 견고한 모듈형 I/O를 통해 가동 시간을 개선하고, 가동 중단 시간을 최소화하며, 설치 공간을 최적화하십시오. \r\n\r\n자세히 알아보기 : https://ab.rockwellautomation.com/ko/IO/In-Cabinet-Modular/FLEX-5000-IO-Modules","name":"FLEX 5000™ I/O 기술 정보","transcript":"","uploadDate":"2018-08-01T03:06:45.000Z","duration":"T4M11S","embedUrl":"https://play.vidyard.com/t8vRsdSqZebFfYXQiR8xCs","height":360,"width":640};
  ...
  function check_for_duplicate_embeds() {
    ...
    for (var i = 0; i < all_scripts.length; ++i) {
      if (all_scripts[i].getAttribute('id') === "vidyard_embed_code_t8vRsdSqZebFfYXQiR8xCs") {
    ...
    if (dupe_scripts.length >= 1) {
      ...
      if (typeof document.querySelectorAll === 'function') {
        ...
        for (var i = 0; i < wrappers.length; ++i) {
          if (wrappers[i].getAttributeNode('onclick').value === "fn_vidyard_t8vRsdSqZebFfYXQiR8xCs();") {
  ...
  if (!embed_script || !embed_script.src) {
    ...
    for (var i in scripts) {
      ...
      if (script.src && /^.+(vidyard\.com).*\/t8vRsdSqZebFfYXQiR8xCs.*/.test(script.src)) {
        embed_script = script;
        break;
      }
    }
  ...
  if (embed_script && embed_script.src) {
    ...
    vidyard_t8vRsdSqZebFfYXQiR8xCs_raw_params = rawQueryParams;
    ...
    embed_script.src.replace(/(?:[\?&])([^&=]*)=?([^&#]*)/g, function ($0, $1, $2) {
      if ($1) {
        switch ($1) {
          ...
          case 'vylegacy':
            vidyard_t8vRsdSqZebFfYXQiR8xCs_params[$1] = (parseInt($2, 10) !== 0);
            break;
          case 'chapter':
            vidyard_t8vRsdSqZebFfYXQiR8xCs_params[$1] = parseInt($2, 10);
            break;
          case 'width':
          ...
          case 'height':
            ...
            break;
          case 'X-VY-Signature':
            vidyard_secure_t8vRsdSqZebFfYXQiR8xCs = true;
          default:
            vidyard_t8vRsdSqZebFfYXQiR8xCs_params[$1] = $2;
            break;
        }
      }
    });
  ...
  if (!vidyard_t8vRsdSqZebFfYXQiR8xCs_params.type || vidyard_t8vRsdSqZebFfYXQiR8xCs_params.type !== 'lightbox') {
    vidyard_t8vRsdSqZebFfYXQiR8xCs_params.type = 'inline';
  }

  // Determine Player Settings which effect embed
  var multi_chapter = false && !(typeof vidyard_t8vRsdSqZebFfYXQiR8xCs_params.hide_html5_playlist !== 'undefined' && vidyard_t8vRsdSqZebFfYXQiR8xCs_params.hide_html5_playlist);
  var playlist_always_open = false;
  if (typeof vidyard_t8vRsdSqZebFfYXQiR8xCs_params.playlist_always_open !== 'undefined') {
    playlist_always_open = vidyard_t8vRsdSqZebFfYXQiR8xCs_params.playlist_always_open;
  }
  ...
  if (query_player_width && query_player_width > 0 && query_player_height && query_player_height > 0) {
    vidyard_player_width_t8vRsdSqZebFfYXQiR8xCs = query_player_width;
    vidyard_player_height_t8vRsdSqZebFfYXQiR8xCs = query_player_height;
  } else if (query_player_width && query_player_width > 0) {
    query_player_height = (query_player_width * vidyard_player_height_t8vRsdSqZebFfYXQiR8xCs) / vidyard_player_width_t8vRsdSqZebFfYXQiR8xCs;
    vidyard_player_width_t8vRsdSqZebFfYXQiR8xCs = query_player_width;
    vidyard_player_height_t8vRsdSqZebFfYXQiR8xCs = query_player_height;
  } else if (query_player_height && query_player_height > 0) {
    query_player_width = (query_player_height * vidyard_player_width_t8vRsdSqZebFfYXQiR8xCs) / vidyard_player_height_t8vRsdSqZebFfYXQiR8xCs;
    vidyard_player_width_t8vRsdSqZebFfYXQiR8xCs = query_player_width;
    vidyard_player_height_t8vRsdSqZebFfYXQiR8xCs = query_player_height;
  }

  var iframe_container = document.getElementById("vidyard_t8vRsdSqZebFfYXQiR8xCs");
  ...
  if (!iframe_container && vidyard_t8vRsdSqZebFfYXQiR8xCs_params.type === 'inline') {
    iframe_container = document.createElement("span");
    iframe_container.id = 'vidyard_t8vRsdSqZebFfYXQiR8xCs';
  ...
  if (vidyard_secure_t8vRsdSqZebFfYXQiR8xCs === false) {
    // Get cookie data/ids from various MAPs/CRMs
    var pardot_id = fn_getCookie("pardot");
    if (pardot_id) {
      vidyard_t8vRsdSqZebFfYXQiR8xCs_params.pardot_id = pardot_id;
    }

    var hubspot_id = fn_getCookie("hubspotutk");
    if (hubspot_id) {
      vidyard_t8vRsdSqZebFfYXQiR8xCs_params.hubspot_id = hubspot_id;
    }
    ...
    if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
      vidyard_t8vRsdSqZebFfYXQiR8xCs_params.marketo_id = encodeURIComponent(marketo_id);
    }

    var dreamforce_id = fn_getCookie("vy_dreamforce");
    if (dreamforce_id) {
      vidyard_t8vRsdSqZebFfYXQiR8xCs_params.dreamforce_id = encodeURIComponent(dreamforce_id);
    }
    ...
    if (vycustom_id !== "") {
      vidyard_t8vRsdSqZebFfYXQiR8xCs_params.custom_id = vycustom_id;
    }

    // Gets the vyemail query string param, which connects views to an email entered
    var vyemail = get_parameter_by_name('vyemail');
    if (vyemail !== "") {
      vidyard_t8vRsdSqZebFfYXQiR8xCs_params.email = vyemail;
    }

    var vyac = get_parameter_by_name('vyac');
    var vyplayer = get_parameter_by_name('vyplayer');
    if (vyplayer === 't8vRsdSqZebFfYXQiR8xCs') {
      vidyard_t8vRsdSqZebFfYXQiR8xCs_params.access_code = vyac;
    }
  }
  ...
  if (navigator.userAgent.toLowerCase().match(/ip(ad|od|hone)|android/)) {
    vidyard_html5_t8vRsdSqZebFfYXQiR8xCs = true;
    vidyard_t8vRsdSqZebFfYXQiR8xCs_params.disable_popouts = true;
  }
  ...
  function create_embed() {
    ...
    var integration_tracking_check = function() {
      if (playerReady) {
        ...
        window.addEventListener('message', function callback(e) {
          ...
          if (data.event === "ready" && data.uuid === "t8vRsdSqZebFfYXQiR8xCs") {
    ...
    var integration_tracking_check_original = function () {
      ...
      if (!referringUrlSent && vidyard_secure_t8vRsdSqZebFfYXQiR8xCs === true && vidyard_t8vRsdSqZebFfYXQiR8xCs_params.referring_url) {
        var playerFrame = Vidyard.Helpers.getPlayerIFrame('t8vRsdSqZebFfYXQiR8xCs');
        var message = {
          'event': 'parentReferer',
          'uuid': 't8vRsdSqZebFfYXQiR8xCs',
          'data': {
            'value': vidyard_t8vRsdSqZebFfYXQiR8xCs_params.referring_url
          }
        }
      ...
      if (!vidyard_t8vRsdSqZebFfYXQiR8xCs_params.pardot_id) {
        var pardot_id = fn_getCookie("pardot");
        if (pardot_id) {
          vidyard_t8vRsdSqZebFfYXQiR8xCs_params.pardot_id = pardot_id;
      ...
      if (!vidyard_t8vRsdSqZebFfYXQiR8xCs_params.hubspot_id) {
        var hubspot_id = fn_getCookie("hubspotutk");
        if (hubspot_id) {
          vidyard_t8vRsdSqZebFfYXQiR8xCs_params.hubspot_id = hubspot_id;
      ...
      if (!vidyard_t8vRsdSqZebFfYXQiR8xCs_params.dreamforce_id) {
        var dreamforce_id = fn_getCookie("vy_dreamforce");
        if (dreamforce_id) {
          dreamforce_id = encodeURIComponent(dreamforce_id);
          vidyard_t8vRsdSqZebFfYXQiR8xCs_params.dreamforce_id = dreamforce_id;
          messages.push({'data': {'type': 'dreamforce', 'value': dreamforce_id}});
        }
      }
      if (!vidyard_t8vRsdSqZebFfYXQiR8xCs_params.marketo_id) {
        var marketo_id = fn_getCookie("_mkto_trk");
        var munchkin_id = '';
        
        if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
          marketo_id = encodeURIComponent(marketo_id);
          vidyard_t8vRsdSqZebFfYXQiR8xCs_params.marketo_id = marketo_id;
      ...
      if (vidyard_t8vRsdSqZebFfYXQiR8xCs_params.type === "inline" || vidyard_secure_t8vRsdSqZebFfYXQiR8xCs === true) {
        for(var i in messages) {
          messages[i].event = 'associateVisitor';
          messages[i].uuid = 't8vRsdSqZebFfYXQiR8xCs';
          Vidyard.Helpers.getPlayerIFrame('t8vRsdSqZebFfYXQiR8xCs').contentWindow.postMessage(JSON.stringify(messages[i]), window.location.protocol + '//play.vidyard.com');
        }
      }
    ...
    vidyard_integration_check_t8vRsdSqZebFfYXQiR8xCs = integration_tracking_check;

    // Display the inline or lightbox
    if (vidyard_t8vRsdSqZebFfYXQiR8xCs_params.type === "inline") {
      // Load the iframe
      iframe_container.innerHTML = fn_vidyard_build_iframe_t8vRsdSqZebFfYXQiR8xCs(false, vidyard_player_width_t8vRsdSqZebFfYXQiR8xCs, vidyard_player_height_t8vRsdSqZebFfYXQiR8xCs);

      Vidyard.Helpers.addListener(
        'load',
        'onload',
        function() {
          fn_vidyard_iframe_after_load_t8vRsdSqZebFfYXQiR8xCs();
          integration_tracking_check();
        },
        Vidyard.Helpers.getPlayerIFrame('t8vRsdSqZebFfYXQiR8xCs')
      );

    } else if (vidyard_t8vRsdSqZebFfYXQiR8xCs_params.type === "lightbox") {
      ...
      if (document.readyState === 'complete') {
        if (!vidyard_secure_t8vRsdSqZebFfYXQiR8xCs) {
          integration_tracking_check();
        }
  ...
  Vidyard._players['t8vRsdSqZebFfYXQiR8xCs'] = '';
}());
...
var vyContactCallback = function(data) {
  ...
  vidyard_t8vRsdSqZebFfYXQiR8xCs_params.marketo_lead = marketoData;
  ...
  // or tried again after IFrame loads in fn_vidyard_iframe_after_load_t8vRsdSqZebFfYXQiR8xCs
  playerIframe = Vidyard.Helpers.getPlayerIFrame('t8vRsdSqZebFfYXQiR8xCs');
  if ( playerIframe ) {
    message = {'data': {'type': 'marketo', 'value': marketoData}};
    message.event = 'identifyVisitor';
    message.uuid = 't8vRsdSqZebFfYXQiR8xCs';
...
(function () {
  function async_load() {
    var contactId = vidyard_t8vRsdSqZebFfYXQiR8xCs_params.marketo_id;
    if (!contactId) {
    ...
    var url = window.location.protocol + '//play.vidyard.com'
        + '/v3/t8vRsdSqZebFfYXQiR8xCs/contact/' + encodeURIComponent(contactId) + '/marketoContact.js';
...
function fn_vidyard_build_iframe_t8vRsdSqZebFfYXQiR8xCs(lightbox, iframe_width, iframe_height) {
  if (lightbox) {
    vidyard_t8vRsdSqZebFfYXQiR8xCs_params.autoplay = true;
  }

  var referring_url = encodeURIComponent(document.referrer);
  if (referring_url) {
    vidyard_t8vRsdSqZebFfYXQiR8xCs_params.referring_url = referring_url;
  }
  ...
  if (document.documentMode === 7) {
    ...
    var ie7RepaintIframe = function() {
      var iframe = Vidyard.Helpers.getPlayerIFrame('t8vRsdSqZebFfYXQiR8xCs');
  ...
  if (vidyard_secure_t8vRsdSqZebFfYXQiR8xCs === true) {
    query_params_string = vidyard_t8vRsdSqZebFfYXQiR8xCs_raw_params;
  } else {
    for (var param in vidyard_t8vRsdSqZebFfYXQiR8xCs_params) {
      var param_value = vidyard_t8vRsdSqZebFfYXQiR8xCs_params[param];
      if (param_value === true) {
        param_value = '1';
      }
  ...
  return '<span id="vidyard_span_t8vRsdSqZebFfYXQiR8xCs" style="display: block; margin: auto; position: relative; width: ' + iframe_width + 'px; height: ' + iframe_height + 'px;"><iframe id="vidyard_iframe_t8vRsdSqZebFfYXQiR8xCs" class="vidyard_iframe" src="//play.vidyard.com/t8vRsdSqZebFfYXQiR8xCs?' + query_params_string + '" width="' + iframe_resizer + '" height="100%" title="Video" aria-label="Video" scrolling=no frameborder=0 allowtransparency="true" allowfullscreen allow="autoplay" style="opacity: 0; background-color: transparent; position: absolute; ' + ios_width_fix + ' right: 0px; top: 0px;"></iframe></span>';
}

// --- If the browser is FF, reload the iframe with the given id
function fn_vidyard_iframe_after_load_t8vRsdSqZebFfYXQiR8xCs() {
  var _theframe = Vidyard.Helpers.getPlayerIFrame('t8vRsdSqZebFfYXQiR8xCs');
  if (_theframe) {
    ...
    if (vidyard_secure_t8vRsdSqZebFfYXQiR8xCs === true && vidyard_t8vRsdSqZebFfYXQiR8xCs_params.type === 'lightbox') {
      vidyard_integration_check_t8vRsdSqZebFfYXQiR8xCs();
    }
    ...
    if (vidyard_t8vRsdSqZebFfYXQiR8xCs_params.marketo_lead) {
      var message = {'data': {'type': 'marketo', 'value': vidyard_t8vRsdSqZebFfYXQiR8xCs_params.marketo_lead }};
      message.event = 'identifyVisitor';
      message.uuid = 't8vRsdSqZebFfYXQiR8xCs';
      Vidyard.Helpers.getPlayerIFrame('t8vRsdSqZebFfYXQiR8xCs')
...
function fn_vidyard_t8vRsdSqZebFfYXQiR8xCs() {
  var vidyard = {};
    ...
    return {
      show:function(initial_iframe_settings) {
        var showCore = function () {
          ...
          var width_height = this.lock_to_aspect_ratio(iframe_settings.width, iframe_settings.height, (319 + (vidyard_player_width_t8vRsdSqZebFfYXQiR8xCs / 5)));
      ...
      fill_iframe: function(settings_html_url, has_element, use_post_request, should_animate, width, height) {
        if(has_element) {
          if(iframe_settings.image) {
            ...
            this.set_tinner_size(fn_vidyard_build_iframe_t8vRsdSqZebFfYXQiR8xCs(true, width, height), should_animate, width, height);
          } else {
      ...
      resize: function() {
        ...
        var width_height = v_box.lock_to_aspect_ratio(
            vidyard_player_width_t8vRsdSqZebFfYXQiR8xCs,
            vidyard_player_height_t8vRsdSqZebFfYXQiR8xCs,
            319 + (vidyard_player_width_t8vRsdSqZebFfYXQiR8xCs / 5)
        );

        iframe_settings.width = width_height[0];
        iframe_settings.height = width_height[1];

        var vidyard_iframe = Vidyard.Helpers.getPlayerIFrame('t8vRsdSqZebFfYXQiR8xCs');
        var vidyard_span = document.getElementById('vidyard_span_t8vRsdSqZebFfYXQiR8xCs');
      ...
      fade_element: function(element_to_fade, fading_in, target_alpha) {
        ...
        if (old_opacity == target_alpha) {
          ...
          if (fading_in == -1) {
            ...
            Vidyard.Helpers.getPlayerIFrame('t8vRsdSqZebFfYXQiR8xCs').src = '';
  ...
  vidyard.box.show({
    iframe: '#', width: vidyard_player_width_t8vRsdSqZebFfYXQiR8xCs,
    height: vidyard_player_height_t8vRsdSqZebFfYXQiR8xCs,
    animate: !vidyard_html5_t8vRsdSqZebFfYXQiR8xCs,
    fixed: true
  });
}
