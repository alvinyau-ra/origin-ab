var vidyard_player_width_okp7Pxp8VeUSZhnUg4jcMf = 640;
var vidyard_player_height_okp7Pxp8VeUSZhnUg4jcMf = 360;
var vidyard_html5_okp7Pxp8VeUSZhnUg4jcMf = false;
var vidyard_secure_okp7Pxp8VeUSZhnUg4jcMf = false;
var vidyard_integration_check_okp7Pxp8VeUSZhnUg4jcMf = function() {};
var vidyard_okp7Pxp8VeUSZhnUg4jcMf_params = {};
var vidyard_okp7Pxp8VeUSZhnUg4jcMf_raw_params = '';
...
(function() {
  ...
  var embed_script = document.getElementById('vidyard_embed_code_okp7Pxp8VeUSZhnUg4jcMf');
  var playerDetails = {"keywords":"Micro800コントローラ,Micro870コントローラ,OEM向けマイクロ制御システム,PLC,マイクロアプリケーション,マイクロコントローラ,マイクロ制御システム,概要,機械装置メーカ向けマイクロ制御システム","thumbnailUrl":"https://play.vidyard.com/okp7Pxp8VeUSZhnUg4jcMf.jpg","@context":"http://schema.org/","@type":"VideoObject","id":"okp7Pxp8VeUSZhnUg4jcMf","description":"Micro800®ファミリーの拡張可能で強力な最新コントローラであるMicro870®コントローラをご紹介します。大型のスタンドアロンマシンのアプリケーション向けに設計されたMicro870コントローラは、大容量メモリを搭載し、よりモジュール化されたプログラムやユーザ定義のファンクションブロックを使用できます。\r\n\r\nMicro870コントローラの詳細については、次のWebサイトをご覧ください：\r\nhttps://ab.rockwellautomation.com/ja/Programmable-Controllers/Micro870","name":"Micro870® コントローラの概要","transcript":"","uploadDate":"2018-08-07T04:29:36.000Z","duration":"T1M41S","embedUrl":"https://play.vidyard.com/okp7Pxp8VeUSZhnUg4jcMf","height":360,"width":640};
  ...
  function check_for_duplicate_embeds() {
    ...
    for (var i = 0; i < all_scripts.length; ++i) {
      if (all_scripts[i].getAttribute('id') === "vidyard_embed_code_okp7Pxp8VeUSZhnUg4jcMf") {
    ...
    if (dupe_scripts.length >= 1) {
      ...
      if (typeof document.querySelectorAll === 'function') {
        ...
        for (var i = 0; i < wrappers.length; ++i) {
          if (wrappers[i].getAttributeNode('onclick').value === "fn_vidyard_okp7Pxp8VeUSZhnUg4jcMf();") {
  ...
  if (!embed_script || !embed_script.src) {
    ...
    for (var i in scripts) {
      ...
      if (script.src && /^.+(vidyard\.com).*\/okp7Pxp8VeUSZhnUg4jcMf.*/.test(script.src)) {
        embed_script = script;
        break;
      }
    }
  ...
  if (embed_script && embed_script.src) {
    ...
    vidyard_okp7Pxp8VeUSZhnUg4jcMf_raw_params = rawQueryParams;
    ...
    embed_script.src.replace(/(?:[\?&])([^&=]*)=?([^&#]*)/g, function ($0, $1, $2) {
      if ($1) {
        switch ($1) {
          ...
          case 'vylegacy':
            vidyard_okp7Pxp8VeUSZhnUg4jcMf_params[$1] = (parseInt($2, 10) !== 0);
            break;
          case 'chapter':
            vidyard_okp7Pxp8VeUSZhnUg4jcMf_params[$1] = parseInt($2, 10);
            break;
          case 'width':
          ...
          case 'height':
            ...
            break;
          case 'X-VY-Signature':
            vidyard_secure_okp7Pxp8VeUSZhnUg4jcMf = true;
          default:
            vidyard_okp7Pxp8VeUSZhnUg4jcMf_params[$1] = $2;
            break;
        }
      }
    });
  ...
  if (!vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.type || vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.type !== 'lightbox') {
    vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.type = 'inline';
  }

  // Determine Player Settings which effect embed
  var multi_chapter = false && !(typeof vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.hide_html5_playlist !== 'undefined' && vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.hide_html5_playlist);
  var playlist_always_open = false;
  if (typeof vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.playlist_always_open !== 'undefined') {
    playlist_always_open = vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.playlist_always_open;
  }
  ...
  if (query_player_width && query_player_width > 0 && query_player_height && query_player_height > 0) {
    vidyard_player_width_okp7Pxp8VeUSZhnUg4jcMf = query_player_width;
    vidyard_player_height_okp7Pxp8VeUSZhnUg4jcMf = query_player_height;
  } else if (query_player_width && query_player_width > 0) {
    query_player_height = (query_player_width * vidyard_player_height_okp7Pxp8VeUSZhnUg4jcMf) / vidyard_player_width_okp7Pxp8VeUSZhnUg4jcMf;
    vidyard_player_width_okp7Pxp8VeUSZhnUg4jcMf = query_player_width;
    vidyard_player_height_okp7Pxp8VeUSZhnUg4jcMf = query_player_height;
  } else if (query_player_height && query_player_height > 0) {
    query_player_width = (query_player_height * vidyard_player_width_okp7Pxp8VeUSZhnUg4jcMf) / vidyard_player_height_okp7Pxp8VeUSZhnUg4jcMf;
    vidyard_player_width_okp7Pxp8VeUSZhnUg4jcMf = query_player_width;
    vidyard_player_height_okp7Pxp8VeUSZhnUg4jcMf = query_player_height;
  }

  var iframe_container = document.getElementById("vidyard_okp7Pxp8VeUSZhnUg4jcMf");
  ...
  if (!iframe_container && vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.type === 'inline') {
    iframe_container = document.createElement("span");
    iframe_container.id = 'vidyard_okp7Pxp8VeUSZhnUg4jcMf';
  ...
  if (vidyard_secure_okp7Pxp8VeUSZhnUg4jcMf === false) {
    // Get cookie data/ids from various MAPs/CRMs
    var pardot_id = fn_getCookie("pardot");
    if (pardot_id) {
      vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.pardot_id = pardot_id;
    }

    var hubspot_id = fn_getCookie("hubspotutk");
    if (hubspot_id) {
      vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.hubspot_id = hubspot_id;
    }
    ...
    if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
      vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.marketo_id = encodeURIComponent(marketo_id);
    }

    var dreamforce_id = fn_getCookie("vy_dreamforce");
    if (dreamforce_id) {
      vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.dreamforce_id = encodeURIComponent(dreamforce_id);
    }
    ...
    if (vycustom_id !== "") {
      vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.custom_id = vycustom_id;
    }

    // Gets the vyemail query string param, which connects views to an email entered
    var vyemail = get_parameter_by_name('vyemail');
    if (vyemail !== "") {
      vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.email = vyemail;
    }

    var vyac = get_parameter_by_name('vyac');
    var vyplayer = get_parameter_by_name('vyplayer');
    if (vyplayer === 'okp7Pxp8VeUSZhnUg4jcMf') {
      vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.access_code = vyac;
    }
  }
  ...
  if (navigator.userAgent.toLowerCase().match(/ip(ad|od|hone)|android/)) {
    vidyard_html5_okp7Pxp8VeUSZhnUg4jcMf = true;
    vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.disable_popouts = true;
  }
  ...
  function create_embed() {
    ...
    var integration_tracking_check = function() {
      if (playerReady) {
        ...
        window.addEventListener('message', function callback(e) {
          ...
          if (data.event === "ready" && data.uuid === "okp7Pxp8VeUSZhnUg4jcMf") {
    ...
    var integration_tracking_check_original = function () {
      ...
      if (!referringUrlSent && vidyard_secure_okp7Pxp8VeUSZhnUg4jcMf === true && vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.referring_url) {
        var playerFrame = Vidyard.Helpers.getPlayerIFrame('okp7Pxp8VeUSZhnUg4jcMf');
        var message = {
          'event': 'parentReferer',
          'uuid': 'okp7Pxp8VeUSZhnUg4jcMf',
          'data': {
            'value': vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.referring_url
          }
        }
      ...
      if (!vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.pardot_id) {
        var pardot_id = fn_getCookie("pardot");
        if (pardot_id) {
          vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.pardot_id = pardot_id;
      ...
      if (!vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.hubspot_id) {
        var hubspot_id = fn_getCookie("hubspotutk");
        if (hubspot_id) {
          vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.hubspot_id = hubspot_id;
      ...
      if (!vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.dreamforce_id) {
        var dreamforce_id = fn_getCookie("vy_dreamforce");
        if (dreamforce_id) {
          dreamforce_id = encodeURIComponent(dreamforce_id);
          vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.dreamforce_id = dreamforce_id;
          messages.push({'data': {'type': 'dreamforce', 'value': dreamforce_id}});
        }
      }
      if (!vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.marketo_id) {
        var marketo_id = fn_getCookie("_mkto_trk");
        var munchkin_id = '';
        
        if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
          marketo_id = encodeURIComponent(marketo_id);
          vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.marketo_id = marketo_id;
      ...
      if (vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.type === "inline" || vidyard_secure_okp7Pxp8VeUSZhnUg4jcMf === true) {
        for(var i in messages) {
          messages[i].event = 'associateVisitor';
          messages[i].uuid = 'okp7Pxp8VeUSZhnUg4jcMf';
          Vidyard.Helpers.getPlayerIFrame('okp7Pxp8VeUSZhnUg4jcMf').contentWindow.postMessage(JSON.stringify(messages[i]), window.location.protocol + '//play.vidyard.com');
        }
      }
    ...
    vidyard_integration_check_okp7Pxp8VeUSZhnUg4jcMf = integration_tracking_check;

    // Display the inline or lightbox
    if (vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.type === "inline") {
      // Load the iframe
      iframe_container.innerHTML = fn_vidyard_build_iframe_okp7Pxp8VeUSZhnUg4jcMf(false, vidyard_player_width_okp7Pxp8VeUSZhnUg4jcMf, vidyard_player_height_okp7Pxp8VeUSZhnUg4jcMf);

      Vidyard.Helpers.addListener(
        'load',
        'onload',
        function() {
          fn_vidyard_iframe_after_load_okp7Pxp8VeUSZhnUg4jcMf();
          integration_tracking_check();
        },
        Vidyard.Helpers.getPlayerIFrame('okp7Pxp8VeUSZhnUg4jcMf')
      );

    } else if (vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.type === "lightbox") {
      ...
      if (document.readyState === 'complete') {
        if (!vidyard_secure_okp7Pxp8VeUSZhnUg4jcMf) {
          integration_tracking_check();
        }
  ...
  Vidyard._players['okp7Pxp8VeUSZhnUg4jcMf'] = '';
}());
...
var vyContactCallback = function(data) {
  ...
  vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.marketo_lead = marketoData;
  ...
  // or tried again after IFrame loads in fn_vidyard_iframe_after_load_okp7Pxp8VeUSZhnUg4jcMf
  playerIframe = Vidyard.Helpers.getPlayerIFrame('okp7Pxp8VeUSZhnUg4jcMf');
  if ( playerIframe ) {
    message = {'data': {'type': 'marketo', 'value': marketoData}};
    message.event = 'identifyVisitor';
    message.uuid = 'okp7Pxp8VeUSZhnUg4jcMf';
...
(function () {
  function async_load() {
    var contactId = vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.marketo_id;
    if (!contactId) {
    ...
    var url = window.location.protocol + '//play.vidyard.com'
        + '/v3/okp7Pxp8VeUSZhnUg4jcMf/contact/' + encodeURIComponent(contactId) + '/marketoContact.js';
...
function fn_vidyard_build_iframe_okp7Pxp8VeUSZhnUg4jcMf(lightbox, iframe_width, iframe_height) {
  if (lightbox) {
    vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.autoplay = true;
  }

  var referring_url = encodeURIComponent(document.referrer);
  if (referring_url) {
    vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.referring_url = referring_url;
  }
  ...
  if (document.documentMode === 7) {
    ...
    var ie7RepaintIframe = function() {
      var iframe = Vidyard.Helpers.getPlayerIFrame('okp7Pxp8VeUSZhnUg4jcMf');
  ...
  if (vidyard_secure_okp7Pxp8VeUSZhnUg4jcMf === true) {
    query_params_string = vidyard_okp7Pxp8VeUSZhnUg4jcMf_raw_params;
  } else {
    for (var param in vidyard_okp7Pxp8VeUSZhnUg4jcMf_params) {
      var param_value = vidyard_okp7Pxp8VeUSZhnUg4jcMf_params[param];
      if (param_value === true) {
        param_value = '1';
      }
  ...
  return '<span id="vidyard_span_okp7Pxp8VeUSZhnUg4jcMf" style="display: block; margin: auto; position: relative; width: ' + iframe_width + 'px; height: ' + iframe_height + 'px;"><iframe id="vidyard_iframe_okp7Pxp8VeUSZhnUg4jcMf" class="vidyard_iframe" src="//play.vidyard.com/okp7Pxp8VeUSZhnUg4jcMf?' + query_params_string + '" width="' + iframe_resizer + '" height="100%" title="Video" aria-label="Video" scrolling=no frameborder=0 allowtransparency="true" allowfullscreen allow="autoplay" style="opacity: 0; background-color: transparent; position: absolute; ' + ios_width_fix + ' right: 0px; top: 0px;"></iframe></span>';
}

// --- If the browser is FF, reload the iframe with the given id
function fn_vidyard_iframe_after_load_okp7Pxp8VeUSZhnUg4jcMf() {
  var _theframe = Vidyard.Helpers.getPlayerIFrame('okp7Pxp8VeUSZhnUg4jcMf');
  if (_theframe) {
    ...
    if (vidyard_secure_okp7Pxp8VeUSZhnUg4jcMf === true && vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.type === 'lightbox') {
      vidyard_integration_check_okp7Pxp8VeUSZhnUg4jcMf();
    }
    ...
    if (vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.marketo_lead) {
      var message = {'data': {'type': 'marketo', 'value': vidyard_okp7Pxp8VeUSZhnUg4jcMf_params.marketo_lead }};
      message.event = 'identifyVisitor';
      message.uuid = 'okp7Pxp8VeUSZhnUg4jcMf';
      Vidyard.Helpers.getPlayerIFrame('okp7Pxp8VeUSZhnUg4jcMf')
...
function fn_vidyard_okp7Pxp8VeUSZhnUg4jcMf() {
  var vidyard = {};
    ...
    return {
      show:function(initial_iframe_settings) {
        var showCore = function () {
          ...
          var width_height = this.lock_to_aspect_ratio(iframe_settings.width, iframe_settings.height, (319 + (vidyard_player_width_okp7Pxp8VeUSZhnUg4jcMf / 5)));
      ...
      fill_iframe: function(settings_html_url, has_element, use_post_request, should_animate, width, height) {
        if(has_element) {
          if(iframe_settings.image) {
            ...
            this.set_tinner_size(fn_vidyard_build_iframe_okp7Pxp8VeUSZhnUg4jcMf(true, width, height), should_animate, width, height);
          } else {
      ...
      resize: function() {
        ...
        var width_height = v_box.lock_to_aspect_ratio(
            vidyard_player_width_okp7Pxp8VeUSZhnUg4jcMf,
            vidyard_player_height_okp7Pxp8VeUSZhnUg4jcMf,
            319 + (vidyard_player_width_okp7Pxp8VeUSZhnUg4jcMf / 5)
        );

        iframe_settings.width = width_height[0];
        iframe_settings.height = width_height[1];

        var vidyard_iframe = Vidyard.Helpers.getPlayerIFrame('okp7Pxp8VeUSZhnUg4jcMf');
        var vidyard_span = document.getElementById('vidyard_span_okp7Pxp8VeUSZhnUg4jcMf');
      ...
      fade_element: function(element_to_fade, fading_in, target_alpha) {
        ...
        if (old_opacity == target_alpha) {
          ...
          if (fading_in == -1) {
            ...
            Vidyard.Helpers.getPlayerIFrame('okp7Pxp8VeUSZhnUg4jcMf').src = '';
  ...
  vidyard.box.show({
    iframe: '#', width: vidyard_player_width_okp7Pxp8VeUSZhnUg4jcMf,
    height: vidyard_player_height_okp7Pxp8VeUSZhnUg4jcMf,
    animate: !vidyard_html5_okp7Pxp8VeUSZhnUg4jcMf,
    fixed: true
  });
}
