var vidyard_player_width_xF4wM3MVwTdCLRwAxUjtpA = 640;
var vidyard_player_height_xF4wM3MVwTdCLRwAxUjtpA = 360;
var vidyard_html5_xF4wM3MVwTdCLRwAxUjtpA = false;
var vidyard_secure_xF4wM3MVwTdCLRwAxUjtpA = false;
var vidyard_integration_check_xF4wM3MVwTdCLRwAxUjtpA = function() {};
var vidyard_xF4wM3MVwTdCLRwAxUjtpA_params = {};
var vidyard_xF4wM3MVwTdCLRwAxUjtpA_raw_params = '';
...
(function() {
  ...
  var embed_script = document.getElementById('vidyard_embed_code_xF4wM3MVwTdCLRwAxUjtpA');
  var playerDetails = {"keywords":"Industrial Control Products,Motor Control Protection,Components","thumbnailUrl":"https://play.vidyard.com/xF4wM3MVwTdCLRwAxUjtpA.jpg","@context":"http://schema.org/","@type":"VideoObject","id":"xF4wM3MVwTdCLRwAxUjtpA","description":"Watch this demonstration of how the E300 Electronic Overload Relay on DeviceNet functions and the new feature that can transition into an E3 or E3+ emulated device (only supported with Series B Control Module). Visit https://ab.rockwellautomation.com/circuit-and-load-protection/lv-motor-protectors/e300-overload-relays for more information on the E300.","name":"E300 on DeviceNet–E3/E3+ Emulation Mode Demonstration","transcript":"Hello. My name is Bruce today and I\nam the global product manager for motor Protection\nSolutions Within the industrial component business\nToday, I'LL be going through a demonstration\nthat shows the E three hundred\noverload relay on device net how\nit functions and acts within the RS networks\nfor device mint interface.\nIn addition,\nI will also demonstrate the new feature\nthat wouldn't\ncombined with the three hundred Siri's be\ncontrol module\nKhun transition the device into what's called\nan E three or e three plus emulation\nmode. Essentially, this mod offers\nthe same features and functionality of une\nthree Yuri three plus device\nfrom a virtual standpoint, so it basically\ntakes the same parameter set of the\nthree hundred.\nBut it puts it into a reduced fashion there by\nmimicking identically\nthe E three every three plus device selected.\nSo what? That let's begin.\nSo as you could see,\nthe three hundred overload really on device Net\nnow shows up as a detectable node\nwithin the RS networks for device net interface\nand on the left hand side here.\nIf you were to select the the motor\noverload,\num, components and with the appropriate\nDDS file, the three hundred will\nnow be will show up as an option there.\nUm, in that tree,\nwhen I click on the device\nand select properties,\nyou can see the\nproperties for the overload really come up\nshowing some basic information about\nthe device itself.\nUm, you can see the catalogue number\nof the device, the type in the firm,\nmy revision\nUp along the top, you have the accessible\ntabs for the device itself.\nSo if we go to the parameters tab,\nthe message will come up that it wants to either upload\nor download from the device or to\nthe device, depending on which stage of the commissioner\nanger. And so, for this particular demonstration,\nwe will upload from the device, since\nwe want to use the parameter set that I already have\nconfigured.\nSo what? The device configured as any three\nhundred, it does take a bit to go through\nand upload each individual parameter\nover the device. That network.\nBut we will give this a few moments until\nit is finished.\nThere we go. And as you can see\nall the parameters that you would see typically\non the Ethan and I P interface or via\nthe Web server are also\nshown here in the forest networks for device\nthat interface\nSo you can view the parameters and Elinor\nlist If you're looking for for one specific\nparameter,\nor is I typically like to see them\nin groups like the Web server\nah, based on functions. So hear this\ngets a little bit better overview of how the parameters\nare organized from a function\nstandpoint, so you can go into\noverload set up and again, you have access to\nchange. You know, all the configurable, the same configurable\nparameters, fella trip class, reset\nmode, anything that was configurable\non the night P interfaces also\nconfigurable on the device interface.\nIn addition, you have access to the E. T\ns file you Khun Sea view that\nfile and I also download a copy of that\nand then offering the device logics perspective.\nYou also have access to that in her face\nhere where you would be able to start the logics\neditor for device logics\nthrough this tab.\nBut if we go back through to\nthe parameter settings here,\nwe could actually no, Like I said, configure this\ndevice\nas an emulated e three or e three\nplus device\nand it's a fairly easy parameter. I remember,\nin my opinion, because it is parameter three\nhundred. So if we navigate it\nthat parameter specifically,\nyou will see that the Emily Russian mode is\npresently disabled.\nBut if we go to enable it, were able\nto select whichever E three\nor e three plus device we wish.\nAnd what's nice is in parentheses. It actually shows\nyou what size of that\ncomponent is that you're looking to emulate,\nmeaning you don't necessarily have to remember the exact\ncatalog string or part number,\num, something that's, you know, not less intuitive. That\nway you could basically pick based on the size\nof the E three or you three plus device that you have\none other thing to note about this\nbased on the three hundred components that you\nhave selected.\n...\n","uploadDate":"2019-05-16T14:51:41.000Z","duration":"T8M43S","embedUrl":"https://play.vidyard.com/xF4wM3MVwTdCLRwAxUjtpA","height":360,"width":640};
  ...
  function check_for_duplicate_embeds() {
    ...
    for (var i = 0; i < all_scripts.length; ++i) {
      if (all_scripts[i].getAttribute('id') === "vidyard_embed_code_xF4wM3MVwTdCLRwAxUjtpA") {
    ...
    if (dupe_scripts.length >= 1) {
      ...
      if (typeof document.querySelectorAll === 'function') {
        ...
        for (var i = 0; i < wrappers.length; ++i) {
          if (wrappers[i].getAttributeNode('onclick').value === "fn_vidyard_xF4wM3MVwTdCLRwAxUjtpA();") {
  ...
  if (!embed_script || !embed_script.src) {
    ...
    for (var i in scripts) {
      ...
      if (script.src && /^.+(vidyard\.com).*\/xF4wM3MVwTdCLRwAxUjtpA.*/.test(script.src)) {
        embed_script = script;
        break;
      }
    }
  ...
  if (embed_script && embed_script.src) {
    ...
    vidyard_xF4wM3MVwTdCLRwAxUjtpA_raw_params = rawQueryParams;
    ...
    embed_script.src.replace(/(?:[\?&])([^&=]*)=?([^&#]*)/g, function ($0, $1, $2) {
      if ($1) {
        switch ($1) {
          ...
          case 'vylegacy':
            vidyard_xF4wM3MVwTdCLRwAxUjtpA_params[$1] = (parseInt($2, 10) !== 0);
            break;
          case 'chapter':
            vidyard_xF4wM3MVwTdCLRwAxUjtpA_params[$1] = parseInt($2, 10);
            break;
          case 'width':
          ...
          case 'height':
            ...
            break;
          case 'X-VY-Signature':
            vidyard_secure_xF4wM3MVwTdCLRwAxUjtpA = true;
          default:
            vidyard_xF4wM3MVwTdCLRwAxUjtpA_params[$1] = $2;
            break;
        }
      }
    });
  ...
  if (!vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.type || vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.type !== 'lightbox') {
    vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.type = 'inline';
  }

  // Determine Player Settings which effect embed
  var multi_chapter = false && !(typeof vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.hide_html5_playlist !== 'undefined' && vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.hide_html5_playlist);
  var playlist_always_open = false;
  if (typeof vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.playlist_always_open !== 'undefined') {
    playlist_always_open = vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.playlist_always_open;
  }
  ...
  if (query_player_width && query_player_width > 0 && query_player_height && query_player_height > 0) {
    vidyard_player_width_xF4wM3MVwTdCLRwAxUjtpA = query_player_width;
    vidyard_player_height_xF4wM3MVwTdCLRwAxUjtpA = query_player_height;
  } else if (query_player_width && query_player_width > 0) {
    query_player_height = (query_player_width * vidyard_player_height_xF4wM3MVwTdCLRwAxUjtpA) / vidyard_player_width_xF4wM3MVwTdCLRwAxUjtpA;
    vidyard_player_width_xF4wM3MVwTdCLRwAxUjtpA = query_player_width;
    vidyard_player_height_xF4wM3MVwTdCLRwAxUjtpA = query_player_height;
  } else if (query_player_height && query_player_height > 0) {
    query_player_width = (query_player_height * vidyard_player_width_xF4wM3MVwTdCLRwAxUjtpA) / vidyard_player_height_xF4wM3MVwTdCLRwAxUjtpA;
    vidyard_player_width_xF4wM3MVwTdCLRwAxUjtpA = query_player_width;
    vidyard_player_height_xF4wM3MVwTdCLRwAxUjtpA = query_player_height;
  }

  var iframe_container = document.getElementById("vidyard_xF4wM3MVwTdCLRwAxUjtpA");
  ...
  if (!iframe_container && vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.type === 'inline') {
    iframe_container = document.createElement("span");
    iframe_container.id = 'vidyard_xF4wM3MVwTdCLRwAxUjtpA';
  ...
  if (vidyard_secure_xF4wM3MVwTdCLRwAxUjtpA === false) {
    // Get cookie data/ids from various MAPs/CRMs
    var pardot_id = fn_getCookie("pardot");
    if (pardot_id) {
      vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.pardot_id = pardot_id;
    }

    var hubspot_id = fn_getCookie("hubspotutk");
    if (hubspot_id) {
      vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.hubspot_id = hubspot_id;
    }
    ...
    if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
      vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.marketo_id = encodeURIComponent(marketo_id);
    }

    var dreamforce_id = fn_getCookie("vy_dreamforce");
    if (dreamforce_id) {
      vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.dreamforce_id = encodeURIComponent(dreamforce_id);
    }
    ...
    if (vycustom_id !== "") {
      vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.custom_id = vycustom_id;
    }

    // Gets the vyemail query string param, which connects views to an email entered
    var vyemail = get_parameter_by_name('vyemail');
    if (vyemail !== "") {
      vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.email = vyemail;
    }

    var vyac = get_parameter_by_name('vyac');
    var vyplayer = get_parameter_by_name('vyplayer');
    if (vyplayer === 'xF4wM3MVwTdCLRwAxUjtpA') {
      vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.access_code = vyac;
    }
  }
  ...
  if (navigator.userAgent.toLowerCase().match(/ip(ad|od|hone)|android/)) {
    vidyard_html5_xF4wM3MVwTdCLRwAxUjtpA = true;
    vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.disable_popouts = true;
  }
  ...
  function create_embed() {
    ...
    var integration_tracking_check = function() {
      if (playerReady) {
        ...
        window.addEventListener('message', function callback(e) {
          ...
          if (data.event === "ready" && data.uuid === "xF4wM3MVwTdCLRwAxUjtpA") {
    ...
    var integration_tracking_check_original = function () {
      ...
      if (!referringUrlSent && vidyard_secure_xF4wM3MVwTdCLRwAxUjtpA === true && vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.referring_url) {
        var playerFrame = Vidyard.Helpers.getPlayerIFrame('xF4wM3MVwTdCLRwAxUjtpA');
        var message = {
          'event': 'parentReferer',
          'uuid': 'xF4wM3MVwTdCLRwAxUjtpA',
          'data': {
            'value': vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.referring_url
          }
        }
      ...
      if (!vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.pardot_id) {
        var pardot_id = fn_getCookie("pardot");
        if (pardot_id) {
          vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.pardot_id = pardot_id;
      ...
      if (!vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.hubspot_id) {
        var hubspot_id = fn_getCookie("hubspotutk");
        if (hubspot_id) {
          vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.hubspot_id = hubspot_id;
      ...
      if (!vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.dreamforce_id) {
        var dreamforce_id = fn_getCookie("vy_dreamforce");
        if (dreamforce_id) {
          dreamforce_id = encodeURIComponent(dreamforce_id);
          vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.dreamforce_id = dreamforce_id;
          messages.push({'data': {'type': 'dreamforce', 'value': dreamforce_id}});
        }
      }
      if (!vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.marketo_id) {
        var marketo_id = fn_getCookie("_mkto_trk");
        var munchkin_id = '';
        
        if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
          marketo_id = encodeURIComponent(marketo_id);
          vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.marketo_id = marketo_id;
        ...
        if (!eloquaSent &&
            vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.eloqua_id &&
            ((vidyard_secure_xF4wM3MVwTdCLRwAxUjtpA === true && vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.type === 'lightbox') ||
             (vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.type === 'inline'))) {
          eloquaSent = true;
          messages.push({'data': {'type': 'eloqua', 'value': vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.eloqua_id}});
        }
      

      // inline embeds are ready to receive api requests
      // lightbox will pass data through querystring params
      if (vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.type === "inline" || vidyard_secure_xF4wM3MVwTdCLRwAxUjtpA === true) {
        for(var i in messages) {
          messages[i].event = 'associateVisitor';
          messages[i].uuid = 'xF4wM3MVwTdCLRwAxUjtpA';
          Vidyard.Helpers.getPlayerIFrame('xF4wM3MVwTdCLRwAxUjtpA').contentWindow.postMessage(JSON.stringify(messages[i]), window.location.protocol + '//play.vidyard.com');
        }
      }
    ...
    vidyard_integration_check_xF4wM3MVwTdCLRwAxUjtpA = integration_tracking_check;

    // Display the inline or lightbox
    if (vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.type === "inline") {
      // Load the iframe
      iframe_container.innerHTML = fn_vidyard_build_iframe_xF4wM3MVwTdCLRwAxUjtpA(false, vidyard_player_width_xF4wM3MVwTdCLRwAxUjtpA, vidyard_player_height_xF4wM3MVwTdCLRwAxUjtpA);

      Vidyard.Helpers.addListener(
        'load',
        'onload',
        function() {
          fn_vidyard_iframe_after_load_xF4wM3MVwTdCLRwAxUjtpA();
          integration_tracking_check();
        },
        Vidyard.Helpers.getPlayerIFrame('xF4wM3MVwTdCLRwAxUjtpA')
      );

    } else if (vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.type === "lightbox") {
      ...
      if (document.readyState === 'complete') {
        if (!vidyard_secure_xF4wM3MVwTdCLRwAxUjtpA) {
          integration_tracking_check();
        }
    ...
    function getEloquaCookie() {
      if (timerId !== null) {
        ...
        if (typeof this.GetElqCustomerGUID === 'function') {
          ...
          vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.eloqua_id = eloqua_id;
          return;
        } else if (this._elqQ) {
  ...
  Vidyard._players['xF4wM3MVwTdCLRwAxUjtpA'] = '';
}());
...
var vyContactCallback = function(data) {
  ...
  vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.marketo_lead = marketoData;
  ...
  // or tried again after IFrame loads in fn_vidyard_iframe_after_load_xF4wM3MVwTdCLRwAxUjtpA
  playerIframe = Vidyard.Helpers.getPlayerIFrame('xF4wM3MVwTdCLRwAxUjtpA');
  if ( playerIframe ) {
    message = {'data': {'type': 'marketo', 'value': marketoData}};
    message.event = 'identifyVisitor';
    message.uuid = 'xF4wM3MVwTdCLRwAxUjtpA';
...
(function () {
  function async_load() {
    var contactId = vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.marketo_id;
    if (!contactId) {
    ...
    var url = window.location.protocol + '//play.vidyard.com'
        + '/v3/xF4wM3MVwTdCLRwAxUjtpA/contact/' + encodeURIComponent(contactId) + '/marketoContact.js';
...
function fn_vidyard_build_iframe_xF4wM3MVwTdCLRwAxUjtpA(lightbox, iframe_width, iframe_height) {
  if (lightbox) {
    vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.autoplay = true;
  }

  var referring_url = encodeURIComponent(document.referrer);
  if (referring_url) {
    vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.referring_url = referring_url;
  }
  ...
  if (document.documentMode === 7) {
    ...
    var ie7RepaintIframe = function() {
      var iframe = Vidyard.Helpers.getPlayerIFrame('xF4wM3MVwTdCLRwAxUjtpA');
  ...
  if (vidyard_secure_xF4wM3MVwTdCLRwAxUjtpA === true) {
    query_params_string = vidyard_xF4wM3MVwTdCLRwAxUjtpA_raw_params;
  } else {
    for (var param in vidyard_xF4wM3MVwTdCLRwAxUjtpA_params) {
      var param_value = vidyard_xF4wM3MVwTdCLRwAxUjtpA_params[param];
      if (param_value === true) {
        param_value = '1';
      }
  ...
  return '<span id="vidyard_span_xF4wM3MVwTdCLRwAxUjtpA" style="display: block; margin: auto; position: relative; width: ' + iframe_width + 'px; height: ' + iframe_height + 'px;"><iframe id="vidyard_iframe_xF4wM3MVwTdCLRwAxUjtpA" class="vidyard_iframe" src="//play.vidyard.com/xF4wM3MVwTdCLRwAxUjtpA?' + query_params_string + '" width="' + iframe_resizer + '" height="100%" title="Video" aria-label="Video" scrolling=no frameborder=0 allowtransparency="true" allowfullscreen allow="autoplay" style="opacity: 0; background-color: transparent; position: absolute; ' + ios_width_fix + ' right: 0px; top: 0px;"></iframe></span>';
}

// --- If the browser is FF, reload the iframe with the given id
function fn_vidyard_iframe_after_load_xF4wM3MVwTdCLRwAxUjtpA() {
  var _theframe = Vidyard.Helpers.getPlayerIFrame('xF4wM3MVwTdCLRwAxUjtpA');
  if (_theframe) {
    ...
    if (vidyard_secure_xF4wM3MVwTdCLRwAxUjtpA === true && vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.type === 'lightbox') {
      vidyard_integration_check_xF4wM3MVwTdCLRwAxUjtpA();
    }
    ...
    if (vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.marketo_lead) {
      var message = {'data': {'type': 'marketo', 'value': vidyard_xF4wM3MVwTdCLRwAxUjtpA_params.marketo_lead }};
      message.event = 'identifyVisitor';
      message.uuid = 'xF4wM3MVwTdCLRwAxUjtpA';
      Vidyard.Helpers.getPlayerIFrame('xF4wM3MVwTdCLRwAxUjtpA')
...
function fn_vidyard_xF4wM3MVwTdCLRwAxUjtpA() {
  var vidyard = {};
    ...
    return {
      show:function(initial_iframe_settings) {
        var showCore = function () {
          ...
          var width_height = this.lock_to_aspect_ratio(iframe_settings.width, iframe_settings.height, (319 + (vidyard_player_width_xF4wM3MVwTdCLRwAxUjtpA / 5)));
      ...
      fill_iframe: function(settings_html_url, has_element, use_post_request, should_animate, width, height) {
        if(has_element) {
          if(iframe_settings.image) {
            ...
            this.set_tinner_size(fn_vidyard_build_iframe_xF4wM3MVwTdCLRwAxUjtpA(true, width, height), should_animate, width, height);
          } else {
      ...
      resize: function() {
        ...
        var width_height = v_box.lock_to_aspect_ratio(
            vidyard_player_width_xF4wM3MVwTdCLRwAxUjtpA,
            vidyard_player_height_xF4wM3MVwTdCLRwAxUjtpA,
            319 + (vidyard_player_width_xF4wM3MVwTdCLRwAxUjtpA / 5)
        );

        iframe_settings.width = width_height[0];
        iframe_settings.height = width_height[1];

        var vidyard_iframe = Vidyard.Helpers.getPlayerIFrame('xF4wM3MVwTdCLRwAxUjtpA');
        var vidyard_span = document.getElementById('vidyard_span_xF4wM3MVwTdCLRwAxUjtpA');
      ...
      fade_element: function(element_to_fade, fading_in, target_alpha) {
        ...
        if (old_opacity == target_alpha) {
          ...
          if (fading_in == -1) {
            ...
            Vidyard.Helpers.getPlayerIFrame('xF4wM3MVwTdCLRwAxUjtpA').src = '';
  ...
  vidyard.box.show({
    iframe: '#', width: vidyard_player_width_xF4wM3MVwTdCLRwAxUjtpA,
    height: vidyard_player_height_xF4wM3MVwTdCLRwAxUjtpA,
    animate: !vidyard_html5_xF4wM3MVwTdCLRwAxUjtpA,
    fixed: true
  });
}
