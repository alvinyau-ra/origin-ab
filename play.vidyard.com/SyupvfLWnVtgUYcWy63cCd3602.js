var vidyard_player_width_SyupvfLWnVtgUYcWy63cCd = 640;
var vidyard_player_height_SyupvfLWnVtgUYcWy63cCd = 360;
var vidyard_html5_SyupvfLWnVtgUYcWy63cCd = false;
var vidyard_secure_SyupvfLWnVtgUYcWy63cCd = false;
var vidyard_integration_check_SyupvfLWnVtgUYcWy63cCd = function() {};
var vidyard_SyupvfLWnVtgUYcWy63cCd_params = {};
var vidyard_SyupvfLWnVtgUYcWy63cCd_raw_params = '';
...
(function() {
  ...
  var embed_script = document.getElementById('vidyard_embed_code_SyupvfLWnVtgUYcWy63cCd');
  var playerDetails = {"keywords":"FLEX 5000 I/O,FLEX I/O,I/O 模組,Logix 5000 系列,分散式 I/O 解決方案,安全 I/O 模組,機櫃模組化","thumbnailUrl":"https://play.vidyard.com/SyupvfLWnVtgUYcWy63cCd.jpg","@context":"http://schema.org/","@type":"VideoObject","id":"SyupvfLWnVtgUYcWy63cCd","description":"探索 FLEX 5000 I/O，全新的分散式 I/O 解決方案可滿足從無塵室製造到嚴苛環境的應用需求。這種靈活耐用的模組化 I/O 可增加正常運作時間、大幅縮短停機時間，並達到優化的體積外型。\r\n\r\n深入瞭解：https://ab.rockwellautomation.com/IO/In-Cabinet-Modular/FLEX-5000-IO-Modules","name":"FLEX 5000™ I/O 技術資訊","transcript":"","uploadDate":"2018-08-01T03:06:34.000Z","duration":"T4M11S","embedUrl":"https://play.vidyard.com/SyupvfLWnVtgUYcWy63cCd","height":360,"width":640};
  ...
  function check_for_duplicate_embeds() {
    ...
    for (var i = 0; i < all_scripts.length; ++i) {
      if (all_scripts[i].getAttribute('id') === "vidyard_embed_code_SyupvfLWnVtgUYcWy63cCd") {
    ...
    if (dupe_scripts.length >= 1) {
      ...
      if (typeof document.querySelectorAll === 'function') {
        ...
        for (var i = 0; i < wrappers.length; ++i) {
          if (wrappers[i].getAttributeNode('onclick').value === "fn_vidyard_SyupvfLWnVtgUYcWy63cCd();") {
  ...
  if (!embed_script || !embed_script.src) {
    ...
    for (var i in scripts) {
      ...
      if (script.src && /^.+(vidyard\.com).*\/SyupvfLWnVtgUYcWy63cCd.*/.test(script.src)) {
        embed_script = script;
        break;
      }
    }
  ...
  if (embed_script && embed_script.src) {
    ...
    vidyard_SyupvfLWnVtgUYcWy63cCd_raw_params = rawQueryParams;
    ...
    embed_script.src.replace(/(?:[\?&])([^&=]*)=?([^&#]*)/g, function ($0, $1, $2) {
      if ($1) {
        switch ($1) {
          ...
          case 'vylegacy':
            vidyard_SyupvfLWnVtgUYcWy63cCd_params[$1] = (parseInt($2, 10) !== 0);
            break;
          case 'chapter':
            vidyard_SyupvfLWnVtgUYcWy63cCd_params[$1] = parseInt($2, 10);
            break;
          case 'width':
          ...
          case 'height':
            ...
            break;
          case 'X-VY-Signature':
            vidyard_secure_SyupvfLWnVtgUYcWy63cCd = true;
          default:
            vidyard_SyupvfLWnVtgUYcWy63cCd_params[$1] = $2;
            break;
        }
      }
    });
  ...
  if (!vidyard_SyupvfLWnVtgUYcWy63cCd_params.type || vidyard_SyupvfLWnVtgUYcWy63cCd_params.type !== 'lightbox') {
    vidyard_SyupvfLWnVtgUYcWy63cCd_params.type = 'inline';
  }

  // Determine Player Settings which effect embed
  var multi_chapter = false && !(typeof vidyard_SyupvfLWnVtgUYcWy63cCd_params.hide_html5_playlist !== 'undefined' && vidyard_SyupvfLWnVtgUYcWy63cCd_params.hide_html5_playlist);
  var playlist_always_open = false;
  if (typeof vidyard_SyupvfLWnVtgUYcWy63cCd_params.playlist_always_open !== 'undefined') {
    playlist_always_open = vidyard_SyupvfLWnVtgUYcWy63cCd_params.playlist_always_open;
  }
  ...
  if (query_player_width && query_player_width > 0 && query_player_height && query_player_height > 0) {
    vidyard_player_width_SyupvfLWnVtgUYcWy63cCd = query_player_width;
    vidyard_player_height_SyupvfLWnVtgUYcWy63cCd = query_player_height;
  } else if (query_player_width && query_player_width > 0) {
    query_player_height = (query_player_width * vidyard_player_height_SyupvfLWnVtgUYcWy63cCd) / vidyard_player_width_SyupvfLWnVtgUYcWy63cCd;
    vidyard_player_width_SyupvfLWnVtgUYcWy63cCd = query_player_width;
    vidyard_player_height_SyupvfLWnVtgUYcWy63cCd = query_player_height;
  } else if (query_player_height && query_player_height > 0) {
    query_player_width = (query_player_height * vidyard_player_width_SyupvfLWnVtgUYcWy63cCd) / vidyard_player_height_SyupvfLWnVtgUYcWy63cCd;
    vidyard_player_width_SyupvfLWnVtgUYcWy63cCd = query_player_width;
    vidyard_player_height_SyupvfLWnVtgUYcWy63cCd = query_player_height;
  }

  var iframe_container = document.getElementById("vidyard_SyupvfLWnVtgUYcWy63cCd");
  ...
  if (!iframe_container && vidyard_SyupvfLWnVtgUYcWy63cCd_params.type === 'inline') {
    iframe_container = document.createElement("span");
    iframe_container.id = 'vidyard_SyupvfLWnVtgUYcWy63cCd';
  ...
  if (vidyard_secure_SyupvfLWnVtgUYcWy63cCd === false) {
    // Get cookie data/ids from various MAPs/CRMs
    var pardot_id = fn_getCookie("pardot");
    if (pardot_id) {
      vidyard_SyupvfLWnVtgUYcWy63cCd_params.pardot_id = pardot_id;
    }

    var hubspot_id = fn_getCookie("hubspotutk");
    if (hubspot_id) {
      vidyard_SyupvfLWnVtgUYcWy63cCd_params.hubspot_id = hubspot_id;
    }
    ...
    if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
      vidyard_SyupvfLWnVtgUYcWy63cCd_params.marketo_id = encodeURIComponent(marketo_id);
    }

    var dreamforce_id = fn_getCookie("vy_dreamforce");
    if (dreamforce_id) {
      vidyard_SyupvfLWnVtgUYcWy63cCd_params.dreamforce_id = encodeURIComponent(dreamforce_id);
    }
    ...
    if (vycustom_id !== "") {
      vidyard_SyupvfLWnVtgUYcWy63cCd_params.custom_id = vycustom_id;
    }

    // Gets the vyemail query string param, which connects views to an email entered
    var vyemail = get_parameter_by_name('vyemail');
    if (vyemail !== "") {
      vidyard_SyupvfLWnVtgUYcWy63cCd_params.email = vyemail;
    }

    var vyac = get_parameter_by_name('vyac');
    var vyplayer = get_parameter_by_name('vyplayer');
    if (vyplayer === 'SyupvfLWnVtgUYcWy63cCd') {
      vidyard_SyupvfLWnVtgUYcWy63cCd_params.access_code = vyac;
    }
  }
  ...
  if (navigator.userAgent.toLowerCase().match(/ip(ad|od|hone)|android/)) {
    vidyard_html5_SyupvfLWnVtgUYcWy63cCd = true;
    vidyard_SyupvfLWnVtgUYcWy63cCd_params.disable_popouts = true;
  }
  ...
  function create_embed() {
    ...
    var integration_tracking_check = function() {
      if (playerReady) {
        ...
        window.addEventListener('message', function callback(e) {
          ...
          if (data.event === "ready" && data.uuid === "SyupvfLWnVtgUYcWy63cCd") {
    ...
    var integration_tracking_check_original = function () {
      ...
      if (!referringUrlSent && vidyard_secure_SyupvfLWnVtgUYcWy63cCd === true && vidyard_SyupvfLWnVtgUYcWy63cCd_params.referring_url) {
        var playerFrame = Vidyard.Helpers.getPlayerIFrame('SyupvfLWnVtgUYcWy63cCd');
        var message = {
          'event': 'parentReferer',
          'uuid': 'SyupvfLWnVtgUYcWy63cCd',
          'data': {
            'value': vidyard_SyupvfLWnVtgUYcWy63cCd_params.referring_url
          }
        }
      ...
      if (!vidyard_SyupvfLWnVtgUYcWy63cCd_params.pardot_id) {
        var pardot_id = fn_getCookie("pardot");
        if (pardot_id) {
          vidyard_SyupvfLWnVtgUYcWy63cCd_params.pardot_id = pardot_id;
      ...
      if (!vidyard_SyupvfLWnVtgUYcWy63cCd_params.hubspot_id) {
        var hubspot_id = fn_getCookie("hubspotutk");
        if (hubspot_id) {
          vidyard_SyupvfLWnVtgUYcWy63cCd_params.hubspot_id = hubspot_id;
      ...
      if (!vidyard_SyupvfLWnVtgUYcWy63cCd_params.dreamforce_id) {
        var dreamforce_id = fn_getCookie("vy_dreamforce");
        if (dreamforce_id) {
          dreamforce_id = encodeURIComponent(dreamforce_id);
          vidyard_SyupvfLWnVtgUYcWy63cCd_params.dreamforce_id = dreamforce_id;
          messages.push({'data': {'type': 'dreamforce', 'value': dreamforce_id}});
        }
      }
      if (!vidyard_SyupvfLWnVtgUYcWy63cCd_params.marketo_id) {
        var marketo_id = fn_getCookie("_mkto_trk");
        var munchkin_id = '';
        
        if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
          marketo_id = encodeURIComponent(marketo_id);
          vidyard_SyupvfLWnVtgUYcWy63cCd_params.marketo_id = marketo_id;
      ...
      if (vidyard_SyupvfLWnVtgUYcWy63cCd_params.type === "inline" || vidyard_secure_SyupvfLWnVtgUYcWy63cCd === true) {
        for(var i in messages) {
          messages[i].event = 'associateVisitor';
          messages[i].uuid = 'SyupvfLWnVtgUYcWy63cCd';
          Vidyard.Helpers.getPlayerIFrame('SyupvfLWnVtgUYcWy63cCd').contentWindow.postMessage(JSON.stringify(messages[i]), window.location.protocol + '//play.vidyard.com');
        }
      }
    ...
    vidyard_integration_check_SyupvfLWnVtgUYcWy63cCd = integration_tracking_check;

    // Display the inline or lightbox
    if (vidyard_SyupvfLWnVtgUYcWy63cCd_params.type === "inline") {
      // Load the iframe
      iframe_container.innerHTML = fn_vidyard_build_iframe_SyupvfLWnVtgUYcWy63cCd(false, vidyard_player_width_SyupvfLWnVtgUYcWy63cCd, vidyard_player_height_SyupvfLWnVtgUYcWy63cCd);

      Vidyard.Helpers.addListener(
        'load',
        'onload',
        function() {
          fn_vidyard_iframe_after_load_SyupvfLWnVtgUYcWy63cCd();
          integration_tracking_check();
        },
        Vidyard.Helpers.getPlayerIFrame('SyupvfLWnVtgUYcWy63cCd')
      );

    } else if (vidyard_SyupvfLWnVtgUYcWy63cCd_params.type === "lightbox") {
      ...
      if (document.readyState === 'complete') {
        if (!vidyard_secure_SyupvfLWnVtgUYcWy63cCd) {
          integration_tracking_check();
        }
  ...
  Vidyard._players['SyupvfLWnVtgUYcWy63cCd'] = '';
}());
...
var vyContactCallback = function(data) {
  ...
  vidyard_SyupvfLWnVtgUYcWy63cCd_params.marketo_lead = marketoData;
  ...
  // or tried again after IFrame loads in fn_vidyard_iframe_after_load_SyupvfLWnVtgUYcWy63cCd
  playerIframe = Vidyard.Helpers.getPlayerIFrame('SyupvfLWnVtgUYcWy63cCd');
  if ( playerIframe ) {
    message = {'data': {'type': 'marketo', 'value': marketoData}};
    message.event = 'identifyVisitor';
    message.uuid = 'SyupvfLWnVtgUYcWy63cCd';
...
(function () {
  function async_load() {
    var contactId = vidyard_SyupvfLWnVtgUYcWy63cCd_params.marketo_id;
    if (!contactId) {
    ...
    var url = window.location.protocol + '//play.vidyard.com'
        + '/v3/SyupvfLWnVtgUYcWy63cCd/contact/' + encodeURIComponent(contactId) + '/marketoContact.js';
...
function fn_vidyard_build_iframe_SyupvfLWnVtgUYcWy63cCd(lightbox, iframe_width, iframe_height) {
  if (lightbox) {
    vidyard_SyupvfLWnVtgUYcWy63cCd_params.autoplay = true;
  }

  var referring_url = encodeURIComponent(document.referrer);
  if (referring_url) {
    vidyard_SyupvfLWnVtgUYcWy63cCd_params.referring_url = referring_url;
  }
  ...
  if (document.documentMode === 7) {
    ...
    var ie7RepaintIframe = function() {
      var iframe = Vidyard.Helpers.getPlayerIFrame('SyupvfLWnVtgUYcWy63cCd');
  ...
  if (vidyard_secure_SyupvfLWnVtgUYcWy63cCd === true) {
    query_params_string = vidyard_SyupvfLWnVtgUYcWy63cCd_raw_params;
  } else {
    for (var param in vidyard_SyupvfLWnVtgUYcWy63cCd_params) {
      var param_value = vidyard_SyupvfLWnVtgUYcWy63cCd_params[param];
      if (param_value === true) {
        param_value = '1';
      }
...
function fn_vidyard_iframe_after_load_SyupvfLWnVtgUYcWy63cCd() {
  ...
  if (_theframe) {
    ...
    if (vidyard_secure_SyupvfLWnVtgUYcWy63cCd === true && vidyard_SyupvfLWnVtgUYcWy63cCd_params.type === 'lightbox') {
      vidyard_integration_check_SyupvfLWnVtgUYcWy63cCd();
    }
    ...
    if (vidyard_SyupvfLWnVtgUYcWy63cCd_params.marketo_lead) {
      var message = {'data': {'type': 'marketo', 'value': vidyard_SyupvfLWnVtgUYcWy63cCd_params.marketo_lead }};
      message.event = 'identifyVisitor';
      message.uuid = 'SyupvfLWnVtgUYcWy63cCd';
      Vidyard.Helpers.getPlayerIFrame('SyupvfLWnVtgUYcWy63cCd')
...
function fn_vidyard_SyupvfLWnVtgUYcWy63cCd() {
  var vidyard = {};
    ...
    return {
      show:function(initial_iframe_settings) {
        var showCore = function () {
          ...
          var width_height = this.lock_to_aspect_ratio(iframe_settings.width, iframe_settings.height, (319 + (vidyard_player_width_SyupvfLWnVtgUYcWy63cCd / 5)));
      ...
      fill_iframe: function(settings_html_url, has_element, use_post_request, should_animate, width, height) {
        if(has_element) {
          if(iframe_settings.image) {
            ...
            this.set_tinner_size(fn_vidyard_build_iframe_SyupvfLWnVtgUYcWy63cCd(true, width, height), should_animate, width, height);
          } else {
      ...
      resize: function() {
        ...
        var width_height = v_box.lock_to_aspect_ratio(
            vidyard_player_width_SyupvfLWnVtgUYcWy63cCd,
            vidyard_player_height_SyupvfLWnVtgUYcWy63cCd,
            319 + (vidyard_player_width_SyupvfLWnVtgUYcWy63cCd / 5)
        );

        iframe_settings.width = width_height[0];
        iframe_settings.height = width_height[1];

        var vidyard_iframe = Vidyard.Helpers.getPlayerIFrame('SyupvfLWnVtgUYcWy63cCd');
        var vidyard_span = document.getElementById('vidyard_span_SyupvfLWnVtgUYcWy63cCd');
      ...
      fade_element: function(element_to_fade, fading_in, target_alpha) {
        ...
        if (old_opacity == target_alpha) {
          ...
          if (fading_in == -1) {
            ...
            Vidyard.Helpers.getPlayerIFrame('SyupvfLWnVtgUYcWy63cCd').src = '';
  ...
  vidyard.box.show({
    iframe: '#', width: vidyard_player_width_SyupvfLWnVtgUYcWy63cCd,
    height: vidyard_player_height_SyupvfLWnVtgUYcWy63cCd,
    animate: !vidyard_html5_SyupvfLWnVtgUYcWy63cCd,
    fixed: true
  });
}
