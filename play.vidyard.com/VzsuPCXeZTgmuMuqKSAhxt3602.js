var vidyard_player_width_VzsuPCXeZTgmuMuqKSAhxt = 959;
var vidyard_player_height_VzsuPCXeZTgmuMuqKSAhxt = 360;
var vidyard_html5_VzsuPCXeZTgmuMuqKSAhxt = false;
var vidyard_secure_VzsuPCXeZTgmuMuqKSAhxt = false;
var vidyard_integration_check_VzsuPCXeZTgmuMuqKSAhxt = function() {};
var vidyard_VzsuPCXeZTgmuMuqKSAhxt_params = {};
var vidyard_VzsuPCXeZTgmuMuqKSAhxt_raw_params = '';
...
(function() {
  ...
  var embed_script = document.getElementById('vidyard_embed_code_VzsuPCXeZTgmuMuqKSAhxt');
  var playerDetails = {"keywords":"Connected Components,Micro800,Micro850,shrink sleeving,Allen-Bradley,Rockwell Automation","thumbnailUrl":"https://play.vidyard.com/VzsuPCXeZTgmuMuqKSAhxt.jpg","@context":"http://schema.org/","@type":"VideoObject","id":"VzsuPCXeZTgmuMuqKSAhxt","description":"The animation includes audio explaining how Micro800 controller can be applied on industrial applications such as shrink sleeving machine. For more information on the Micro850 controller featured in the animation, visit http://ab.rockwellautomation.com/Programmable-Controllers/Micro850","name":"Connected Components Solution for Shrink Sleeving Machine","transcript":"","uploadDate":"2017-06-09T03:09:11.000Z","duration":"T2M49S","embedUrl":"https://play.vidyard.com/VzsuPCXeZTgmuMuqKSAhxt","height":360,"width":959};
  ...
  function check_for_duplicate_embeds() {
    ...
    for (var i = 0; i < all_scripts.length; ++i) {
      if (all_scripts[i].getAttribute('id') === "vidyard_embed_code_VzsuPCXeZTgmuMuqKSAhxt") {
    ...
    if (dupe_scripts.length >= 1) {
      ...
      if (typeof document.querySelectorAll === 'function') {
        ...
        for (var i = 0; i < wrappers.length; ++i) {
          if (wrappers[i].getAttributeNode('onclick').value === "fn_vidyard_VzsuPCXeZTgmuMuqKSAhxt();") {
  ...
  if (!embed_script || !embed_script.src) {
    ...
    for (var i in scripts) {
      ...
      if (script.src && /^.+(vidyard\.com).*\/VzsuPCXeZTgmuMuqKSAhxt.*/.test(script.src)) {
        embed_script = script;
        break;
      }
    }
  ...
  if (embed_script && embed_script.src) {
    ...
    vidyard_VzsuPCXeZTgmuMuqKSAhxt_raw_params = rawQueryParams;
    ...
    embed_script.src.replace(/(?:[\?&])([^&=]*)=?([^&#]*)/g, function ($0, $1, $2) {
      if ($1) {
        switch ($1) {
          ...
          case 'vylegacy':
            vidyard_VzsuPCXeZTgmuMuqKSAhxt_params[$1] = (parseInt($2, 10) !== 0);
            break;
          case 'chapter':
            vidyard_VzsuPCXeZTgmuMuqKSAhxt_params[$1] = parseInt($2, 10);
            break;
          case 'width':
          ...
          case 'height':
            ...
            break;
          case 'X-VY-Signature':
            vidyard_secure_VzsuPCXeZTgmuMuqKSAhxt = true;
          default:
            vidyard_VzsuPCXeZTgmuMuqKSAhxt_params[$1] = $2;
            break;
        }
      }
    });
  ...
  if (!vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.type || vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.type !== 'lightbox') {
    vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.type = 'inline';
  }

  // Determine Player Settings which effect embed
  var multi_chapter = false && !(typeof vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.hide_html5_playlist !== 'undefined' && vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.hide_html5_playlist);
  var playlist_always_open = false;
  if (typeof vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.playlist_always_open !== 'undefined') {
    playlist_always_open = vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.playlist_always_open;
  }
  ...
  if (query_player_width && query_player_width > 0 && query_player_height && query_player_height > 0) {
    vidyard_player_width_VzsuPCXeZTgmuMuqKSAhxt = query_player_width;
    vidyard_player_height_VzsuPCXeZTgmuMuqKSAhxt = query_player_height;
  } else if (query_player_width && query_player_width > 0) {
    query_player_height = (query_player_width * vidyard_player_height_VzsuPCXeZTgmuMuqKSAhxt) / vidyard_player_width_VzsuPCXeZTgmuMuqKSAhxt;
    vidyard_player_width_VzsuPCXeZTgmuMuqKSAhxt = query_player_width;
    vidyard_player_height_VzsuPCXeZTgmuMuqKSAhxt = query_player_height;
  } else if (query_player_height && query_player_height > 0) {
    query_player_width = (query_player_height * vidyard_player_width_VzsuPCXeZTgmuMuqKSAhxt) / vidyard_player_height_VzsuPCXeZTgmuMuqKSAhxt;
    vidyard_player_width_VzsuPCXeZTgmuMuqKSAhxt = query_player_width;
    vidyard_player_height_VzsuPCXeZTgmuMuqKSAhxt = query_player_height;
  }

  var iframe_container = document.getElementById("vidyard_VzsuPCXeZTgmuMuqKSAhxt");
  ...
  if (!iframe_container && vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.type === 'inline') {
    iframe_container = document.createElement("span");
    iframe_container.id = 'vidyard_VzsuPCXeZTgmuMuqKSAhxt';
  ...
  if (vidyard_secure_VzsuPCXeZTgmuMuqKSAhxt === false) {
    // Get cookie data/ids from various MAPs/CRMs
    var pardot_id = fn_getCookie("pardot");
    if (pardot_id) {
      vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.pardot_id = pardot_id;
    }

    var hubspot_id = fn_getCookie("hubspotutk");
    if (hubspot_id) {
      vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.hubspot_id = hubspot_id;
    }
    ...
    if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
      vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.marketo_id = encodeURIComponent(marketo_id);
    }

    var dreamforce_id = fn_getCookie("vy_dreamforce");
    if (dreamforce_id) {
      vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.dreamforce_id = encodeURIComponent(dreamforce_id);
    }
    ...
    if (vycustom_id !== "") {
      vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.custom_id = vycustom_id;
    }

    // Gets the vyemail query string param, which connects views to an email entered
    var vyemail = get_parameter_by_name('vyemail');
    if (vyemail !== "") {
      vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.email = vyemail;
    }

    var vyac = get_parameter_by_name('vyac');
    var vyplayer = get_parameter_by_name('vyplayer');
    if (vyplayer === 'VzsuPCXeZTgmuMuqKSAhxt') {
      vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.access_code = vyac;
    }
  }
  ...
  if (navigator.userAgent.toLowerCase().match(/ip(ad|od|hone)|android/)) {
    vidyard_html5_VzsuPCXeZTgmuMuqKSAhxt = true;
    vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.disable_popouts = true;
  }
  ...
  function create_embed() {
    ...
    var integration_tracking_check = function() {
      if (playerReady) {
        ...
        window.addEventListener('message', function callback(e) {
          ...
          if (data.event === "ready" && data.uuid === "VzsuPCXeZTgmuMuqKSAhxt") {
    ...
    var integration_tracking_check_original = function () {
      ...
      if (!referringUrlSent && vidyard_secure_VzsuPCXeZTgmuMuqKSAhxt === true && vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.referring_url) {
        var playerFrame = Vidyard.Helpers.getPlayerIFrame('VzsuPCXeZTgmuMuqKSAhxt');
        var message = {
          'event': 'parentReferer',
          'uuid': 'VzsuPCXeZTgmuMuqKSAhxt',
          'data': {
            'value': vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.referring_url
          }
        }
      ...
      if (!vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.pardot_id) {
        var pardot_id = fn_getCookie("pardot");
        if (pardot_id) {
          vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.pardot_id = pardot_id;
      ...
      if (!vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.hubspot_id) {
        var hubspot_id = fn_getCookie("hubspotutk");
        if (hubspot_id) {
          vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.hubspot_id = hubspot_id;
      ...
      if (!vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.dreamforce_id) {
        var dreamforce_id = fn_getCookie("vy_dreamforce");
        if (dreamforce_id) {
          dreamforce_id = encodeURIComponent(dreamforce_id);
          vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.dreamforce_id = dreamforce_id;
          messages.push({'data': {'type': 'dreamforce', 'value': dreamforce_id}});
        }
      }
      if (!vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.marketo_id) {
        var marketo_id = fn_getCookie("_mkto_trk");
        var munchkin_id = '';
        
        if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
          marketo_id = encodeURIComponent(marketo_id);
          vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.marketo_id = marketo_id;
      ...
      if (vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.type === "inline" || vidyard_secure_VzsuPCXeZTgmuMuqKSAhxt === true) {
        for(var i in messages) {
          messages[i].event = 'associateVisitor';
          messages[i].uuid = 'VzsuPCXeZTgmuMuqKSAhxt';
          Vidyard.Helpers.getPlayerIFrame('VzsuPCXeZTgmuMuqKSAhxt').contentWindow.postMessage(JSON.stringify(messages[i]), window.location.protocol + '//play.vidyard.com');
        }
      }
    ...
    vidyard_integration_check_VzsuPCXeZTgmuMuqKSAhxt = integration_tracking_check;

    // Display the inline or lightbox
    if (vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.type === "inline") {
      // Load the iframe
      iframe_container.innerHTML = fn_vidyard_build_iframe_VzsuPCXeZTgmuMuqKSAhxt(false, vidyard_player_width_VzsuPCXeZTgmuMuqKSAhxt, vidyard_player_height_VzsuPCXeZTgmuMuqKSAhxt);

      Vidyard.Helpers.addListener(
        'load',
        'onload',
        function() {
          fn_vidyard_iframe_after_load_VzsuPCXeZTgmuMuqKSAhxt();
          integration_tracking_check();
        },
        Vidyard.Helpers.getPlayerIFrame('VzsuPCXeZTgmuMuqKSAhxt')
      );

    } else if (vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.type === "lightbox") {
      ...
      if (document.readyState === 'complete') {
        if (!vidyard_secure_VzsuPCXeZTgmuMuqKSAhxt) {
          integration_tracking_check();
        }
  ...
  Vidyard._players['VzsuPCXeZTgmuMuqKSAhxt'] = '';
}());
...
var vyContactCallback = function(data) {
  ...
  vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.marketo_lead = marketoData;
  ...
  // or tried again after IFrame loads in fn_vidyard_iframe_after_load_VzsuPCXeZTgmuMuqKSAhxt
  playerIframe = Vidyard.Helpers.getPlayerIFrame('VzsuPCXeZTgmuMuqKSAhxt');
  if ( playerIframe ) {
    message = {'data': {'type': 'marketo', 'value': marketoData}};
    message.event = 'identifyVisitor';
    message.uuid = 'VzsuPCXeZTgmuMuqKSAhxt';
...
(function () {
  function async_load() {
    var contactId = vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.marketo_id;
    if (!contactId) {
    ...
    var url = window.location.protocol + '//play.vidyard.com'
        + '/v3/VzsuPCXeZTgmuMuqKSAhxt/contact/' + encodeURIComponent(contactId) + '/marketoContact.js';
...
function fn_vidyard_build_iframe_VzsuPCXeZTgmuMuqKSAhxt(lightbox, iframe_width, iframe_height) {
  if (lightbox) {
    vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.autoplay = true;
  }

  var referring_url = encodeURIComponent(document.referrer);
  if (referring_url) {
    vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.referring_url = referring_url;
  }
  ...
  if (document.documentMode === 7) {
    ...
    var ie7RepaintIframe = function() {
      var iframe = Vidyard.Helpers.getPlayerIFrame('VzsuPCXeZTgmuMuqKSAhxt');
  ...
  if (vidyard_secure_VzsuPCXeZTgmuMuqKSAhxt === true) {
    query_params_string = vidyard_VzsuPCXeZTgmuMuqKSAhxt_raw_params;
  } else {
    for (var param in vidyard_VzsuPCXeZTgmuMuqKSAhxt_params) {
      var param_value = vidyard_VzsuPCXeZTgmuMuqKSAhxt_params[param];
      if (param_value === true) {
        param_value = '1';
      }
...
function fn_vidyard_iframe_after_load_VzsuPCXeZTgmuMuqKSAhxt() {
  ...
  if (_theframe) {
    ...
    if (vidyard_secure_VzsuPCXeZTgmuMuqKSAhxt === true && vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.type === 'lightbox') {
      vidyard_integration_check_VzsuPCXeZTgmuMuqKSAhxt();
    }
    ...
    if (vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.marketo_lead) {
      var message = {'data': {'type': 'marketo', 'value': vidyard_VzsuPCXeZTgmuMuqKSAhxt_params.marketo_lead }};
      message.event = 'identifyVisitor';
      message.uuid = 'VzsuPCXeZTgmuMuqKSAhxt';
      Vidyard.Helpers.getPlayerIFrame('VzsuPCXeZTgmuMuqKSAhxt')
...
function fn_vidyard_VzsuPCXeZTgmuMuqKSAhxt() {
  var vidyard = {};
    ...
    return {
      show:function(initial_iframe_settings) {
        var showCore = function () {
          ...
          var width_height = this.lock_to_aspect_ratio(iframe_settings.width, iframe_settings.height, (319 + (vidyard_player_width_VzsuPCXeZTgmuMuqKSAhxt / 5)));
      ...
      fill_iframe: function(settings_html_url, has_element, use_post_request, should_animate, width, height) {
        if(has_element) {
          if(iframe_settings.image) {
            ...
            this.set_tinner_size(fn_vidyard_build_iframe_VzsuPCXeZTgmuMuqKSAhxt(true, width, height), should_animate, width, height);
          } else {
      ...
      resize: function() {
        ...
        var width_height = v_box.lock_to_aspect_ratio(
            vidyard_player_width_VzsuPCXeZTgmuMuqKSAhxt,
            vidyard_player_height_VzsuPCXeZTgmuMuqKSAhxt,
            319 + (vidyard_player_width_VzsuPCXeZTgmuMuqKSAhxt / 5)
        );

        iframe_settings.width = width_height[0];
        iframe_settings.height = width_height[1];

        var vidyard_iframe = Vidyard.Helpers.getPlayerIFrame('VzsuPCXeZTgmuMuqKSAhxt');
        var vidyard_span = document.getElementById('vidyard_span_VzsuPCXeZTgmuMuqKSAhxt');
      ...
      fade_element: function(element_to_fade, fading_in, target_alpha) {
        ...
        if (old_opacity == target_alpha) {
          ...
          if (fading_in == -1) {
            ...
            Vidyard.Helpers.getPlayerIFrame('VzsuPCXeZTgmuMuqKSAhxt').src = '';
  ...
  vidyard.box.show({
    iframe: '#', width: vidyard_player_width_VzsuPCXeZTgmuMuqKSAhxt,
    height: vidyard_player_height_VzsuPCXeZTgmuMuqKSAhxt,
    animate: !vidyard_html5_VzsuPCXeZTgmuMuqKSAhxt,
    fixed: true
  });
}
