var vidyard_player_width_whLu9ete8y4sADcYxMK8iX = 640;
var vidyard_player_height_whLu9ete8y4sADcYxMK8iX = 360;
var vidyard_html5_whLu9ete8y4sADcYxMK8iX = false;
var vidyard_secure_whLu9ete8y4sADcYxMK8iX = false;
var vidyard_integration_check_whLu9ete8y4sADcYxMK8iX = function() {};
var vidyard_whLu9ete8y4sADcYxMK8iX_params = {};
var vidyard_whLu9ete8y4sADcYxMK8iX_raw_params = '';
...
(function() {
  ...
  var embed_script = document.getElementById('vidyard_embed_code_whLu9ete8y4sADcYxMK8iX');
  var playerDetails = {"keywords":"motion control,motors","thumbnailUrl":"https://play.vidyard.com/whLu9ete8y4sADcYxMK8iX.jpg","@context":"http://schema.org/","@type":"VideoObject","id":"whLu9ete8y4sADcYxMK8iX","description":"更多信息请访问：http://ab.rockwellautomation.com/zh/Motion-Control/Kinetix-VP-Servo-Motors\r\n","name":"Kinetix VPC 连续负载伺服电机","transcript":"Kinetix VPC 连续负载伺服电机\n通过强制对流技术扩展功率范围\n任何安装方向都可保持 IP65 防护等级\nL10 轴承使用寿命更长\n内置永磁提供更高的最大转速\n符合 IE4 能效等级\n减小转矩波动\n可现场更换的风扇\n提供 SIL2/PLd 选项的 DSL 编码器\nEnDAT 数字编码器选件\n底部安装座有助于简化安装\n单电缆设计可减少接线\nKinetix 5700 伺服驱动器提供基于\nEtherNet/IP 的集成运动控制\nKinetix VPC 连续负载伺服电机\n","uploadDate":"2017-06-15T17:59:38.000Z","duration":"T2M6S","embedUrl":"https://play.vidyard.com/whLu9ete8y4sADcYxMK8iX","height":360,"width":640};
  ...
  function check_for_duplicate_embeds() {
    ...
    for (var i = 0; i < all_scripts.length; ++i) {
      if (all_scripts[i].getAttribute('id') === "vidyard_embed_code_whLu9ete8y4sADcYxMK8iX") {
    ...
    if (dupe_scripts.length >= 1) {
      ...
      if (typeof document.querySelectorAll === 'function') {
        ...
        for (var i = 0; i < wrappers.length; ++i) {
          if (wrappers[i].getAttributeNode('onclick').value === "fn_vidyard_whLu9ete8y4sADcYxMK8iX();") {
  ...
  if (!embed_script || !embed_script.src) {
    ...
    for (var i in scripts) {
      ...
      if (script.src && /^.+(vidyard\.com).*\/whLu9ete8y4sADcYxMK8iX.*/.test(script.src)) {
        embed_script = script;
        break;
      }
    }
  ...
  if (embed_script && embed_script.src) {
    ...
    vidyard_whLu9ete8y4sADcYxMK8iX_raw_params = rawQueryParams;
    ...
    embed_script.src.replace(/(?:[\?&])([^&=]*)=?([^&#]*)/g, function ($0, $1, $2) {
      if ($1) {
        switch ($1) {
          ...
          case 'vylegacy':
            vidyard_whLu9ete8y4sADcYxMK8iX_params[$1] = (parseInt($2, 10) !== 0);
            break;
          case 'chapter':
            vidyard_whLu9ete8y4sADcYxMK8iX_params[$1] = parseInt($2, 10);
            break;
          case 'width':
          ...
          case 'height':
            ...
            break;
          case 'X-VY-Signature':
            vidyard_secure_whLu9ete8y4sADcYxMK8iX = true;
          default:
            vidyard_whLu9ete8y4sADcYxMK8iX_params[$1] = $2;
            break;
        }
      }
    });
  ...
  if (!vidyard_whLu9ete8y4sADcYxMK8iX_params.type || vidyard_whLu9ete8y4sADcYxMK8iX_params.type !== 'lightbox') {
    vidyard_whLu9ete8y4sADcYxMK8iX_params.type = 'inline';
  }

  // Determine Player Settings which effect embed
  var multi_chapter = false && !(typeof vidyard_whLu9ete8y4sADcYxMK8iX_params.hide_html5_playlist !== 'undefined' && vidyard_whLu9ete8y4sADcYxMK8iX_params.hide_html5_playlist);
  var playlist_always_open = false;
  if (typeof vidyard_whLu9ete8y4sADcYxMK8iX_params.playlist_always_open !== 'undefined') {
    playlist_always_open = vidyard_whLu9ete8y4sADcYxMK8iX_params.playlist_always_open;
  }
  ...
  if (query_player_width && query_player_width > 0 && query_player_height && query_player_height > 0) {
    vidyard_player_width_whLu9ete8y4sADcYxMK8iX = query_player_width;
    vidyard_player_height_whLu9ete8y4sADcYxMK8iX = query_player_height;
  } else if (query_player_width && query_player_width > 0) {
    query_player_height = (query_player_width * vidyard_player_height_whLu9ete8y4sADcYxMK8iX) / vidyard_player_width_whLu9ete8y4sADcYxMK8iX;
    vidyard_player_width_whLu9ete8y4sADcYxMK8iX = query_player_width;
    vidyard_player_height_whLu9ete8y4sADcYxMK8iX = query_player_height;
  } else if (query_player_height && query_player_height > 0) {
    query_player_width = (query_player_height * vidyard_player_width_whLu9ete8y4sADcYxMK8iX) / vidyard_player_height_whLu9ete8y4sADcYxMK8iX;
    vidyard_player_width_whLu9ete8y4sADcYxMK8iX = query_player_width;
    vidyard_player_height_whLu9ete8y4sADcYxMK8iX = query_player_height;
  }

  var iframe_container = document.getElementById("vidyard_whLu9ete8y4sADcYxMK8iX");
  ...
  if (!iframe_container && vidyard_whLu9ete8y4sADcYxMK8iX_params.type === 'inline') {
    iframe_container = document.createElement("span");
    iframe_container.id = 'vidyard_whLu9ete8y4sADcYxMK8iX';
  ...
  if (vidyard_secure_whLu9ete8y4sADcYxMK8iX === false) {
    // Get cookie data/ids from various MAPs/CRMs
    var pardot_id = fn_getCookie("pardot");
    if (pardot_id) {
      vidyard_whLu9ete8y4sADcYxMK8iX_params.pardot_id = pardot_id;
    }

    var hubspot_id = fn_getCookie("hubspotutk");
    if (hubspot_id) {
      vidyard_whLu9ete8y4sADcYxMK8iX_params.hubspot_id = hubspot_id;
    }
    ...
    if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
      vidyard_whLu9ete8y4sADcYxMK8iX_params.marketo_id = encodeURIComponent(marketo_id);
    }

    var dreamforce_id = fn_getCookie("vy_dreamforce");
    if (dreamforce_id) {
      vidyard_whLu9ete8y4sADcYxMK8iX_params.dreamforce_id = encodeURIComponent(dreamforce_id);
    }
    ...
    if (vycustom_id !== "") {
      vidyard_whLu9ete8y4sADcYxMK8iX_params.custom_id = vycustom_id;
    }

    // Gets the vyemail query string param, which connects views to an email entered
    var vyemail = get_parameter_by_name('vyemail');
    if (vyemail !== "") {
      vidyard_whLu9ete8y4sADcYxMK8iX_params.email = vyemail;
    }

    var vyac = get_parameter_by_name('vyac');
    var vyplayer = get_parameter_by_name('vyplayer');
    if (vyplayer === 'whLu9ete8y4sADcYxMK8iX') {
      vidyard_whLu9ete8y4sADcYxMK8iX_params.access_code = vyac;
    }
  }
  ...
  if (navigator.userAgent.toLowerCase().match(/ip(ad|od|hone)|android/)) {
    vidyard_html5_whLu9ete8y4sADcYxMK8iX = true;
    vidyard_whLu9ete8y4sADcYxMK8iX_params.disable_popouts = true;
  }
  ...
  function create_embed() {
    ...
    var integration_tracking_check = function() {
      if (playerReady) {
        ...
        window.addEventListener('message', function callback(e) {
          ...
          if (data.event === "ready" && data.uuid === "whLu9ete8y4sADcYxMK8iX") {
    ...
    var integration_tracking_check_original = function () {
      ...
      if (!referringUrlSent && vidyard_secure_whLu9ete8y4sADcYxMK8iX === true && vidyard_whLu9ete8y4sADcYxMK8iX_params.referring_url) {
        var playerFrame = Vidyard.Helpers.getPlayerIFrame('whLu9ete8y4sADcYxMK8iX');
        var message = {
          'event': 'parentReferer',
          'uuid': 'whLu9ete8y4sADcYxMK8iX',
          'data': {
            'value': vidyard_whLu9ete8y4sADcYxMK8iX_params.referring_url
          }
        }
      ...
      if (!vidyard_whLu9ete8y4sADcYxMK8iX_params.pardot_id) {
        var pardot_id = fn_getCookie("pardot");
        if (pardot_id) {
          vidyard_whLu9ete8y4sADcYxMK8iX_params.pardot_id = pardot_id;
      ...
      if (!vidyard_whLu9ete8y4sADcYxMK8iX_params.hubspot_id) {
        var hubspot_id = fn_getCookie("hubspotutk");
        if (hubspot_id) {
          vidyard_whLu9ete8y4sADcYxMK8iX_params.hubspot_id = hubspot_id;
      ...
      if (!vidyard_whLu9ete8y4sADcYxMK8iX_params.dreamforce_id) {
        var dreamforce_id = fn_getCookie("vy_dreamforce");
        if (dreamforce_id) {
          dreamforce_id = encodeURIComponent(dreamforce_id);
          vidyard_whLu9ete8y4sADcYxMK8iX_params.dreamforce_id = dreamforce_id;
          messages.push({'data': {'type': 'dreamforce', 'value': dreamforce_id}});
        }
      }
      if (!vidyard_whLu9ete8y4sADcYxMK8iX_params.marketo_id) {
        var marketo_id = fn_getCookie("_mkto_trk");
        var munchkin_id = '';
        
        if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
          marketo_id = encodeURIComponent(marketo_id);
          vidyard_whLu9ete8y4sADcYxMK8iX_params.marketo_id = marketo_id;
        ...
        if (!eloquaSent &&
            vidyard_whLu9ete8y4sADcYxMK8iX_params.eloqua_id &&
            ((vidyard_secure_whLu9ete8y4sADcYxMK8iX === true && vidyard_whLu9ete8y4sADcYxMK8iX_params.type === 'lightbox') ||
             (vidyard_whLu9ete8y4sADcYxMK8iX_params.type === 'inline'))) {
          eloquaSent = true;
          messages.push({'data': {'type': 'eloqua', 'value': vidyard_whLu9ete8y4sADcYxMK8iX_params.eloqua_id}});
        }
      

      // inline embeds are ready to receive api requests
      // lightbox will pass data through querystring params
      if (vidyard_whLu9ete8y4sADcYxMK8iX_params.type === "inline" || vidyard_secure_whLu9ete8y4sADcYxMK8iX === true) {
        for(var i in messages) {
          messages[i].event = 'associateVisitor';
          messages[i].uuid = 'whLu9ete8y4sADcYxMK8iX';
          Vidyard.Helpers.getPlayerIFrame('whLu9ete8y4sADcYxMK8iX').contentWindow.postMessage(JSON.stringify(messages[i]), window.location.protocol + '//play.vidyard.com');
        }
      }
    ...
    vidyard_integration_check_whLu9ete8y4sADcYxMK8iX = integration_tracking_check;

    // Display the inline or lightbox
    if (vidyard_whLu9ete8y4sADcYxMK8iX_params.type === "inline") {
      // Load the iframe
      iframe_container.innerHTML = fn_vidyard_build_iframe_whLu9ete8y4sADcYxMK8iX(false, vidyard_player_width_whLu9ete8y4sADcYxMK8iX, vidyard_player_height_whLu9ete8y4sADcYxMK8iX);

      Vidyard.Helpers.addListener(
        'load',
        'onload',
        function() {
          fn_vidyard_iframe_after_load_whLu9ete8y4sADcYxMK8iX();
          integration_tracking_check();
        },
        Vidyard.Helpers.getPlayerIFrame('whLu9ete8y4sADcYxMK8iX')
      );

    } else if (vidyard_whLu9ete8y4sADcYxMK8iX_params.type === "lightbox") {
      ...
      if (document.readyState === 'complete') {
        if (!vidyard_secure_whLu9ete8y4sADcYxMK8iX) {
          integration_tracking_check();
        }
    ...
    function getEloquaCookie() {
      if (timerId !== null) {
        ...
        if (typeof this.GetElqCustomerGUID === 'function') {
          ...
          vidyard_whLu9ete8y4sADcYxMK8iX_params.eloqua_id = eloqua_id;
          return;
        } else if (this._elqQ) {
  ...
  Vidyard._players['whLu9ete8y4sADcYxMK8iX'] = '';
}());
...
var vyContactCallback = function(data) {
  ...
  vidyard_whLu9ete8y4sADcYxMK8iX_params.marketo_lead = marketoData;
  ...
  // or tried again after IFrame loads in fn_vidyard_iframe_after_load_whLu9ete8y4sADcYxMK8iX
  playerIframe = Vidyard.Helpers.getPlayerIFrame('whLu9ete8y4sADcYxMK8iX');
  if ( playerIframe ) {
    message = {'data': {'type': 'marketo', 'value': marketoData}};
    message.event = 'identifyVisitor';
    message.uuid = 'whLu9ete8y4sADcYxMK8iX';
...
(function () {
  function async_load() {
    var contactId = vidyard_whLu9ete8y4sADcYxMK8iX_params.marketo_id;
    if (!contactId) {
    ...
    var url = window.location.protocol + '//play.vidyard.com'
        + '/v3/whLu9ete8y4sADcYxMK8iX/contact/' + encodeURIComponent(contactId) + '/marketoContact.js';
...
function fn_vidyard_build_iframe_whLu9ete8y4sADcYxMK8iX(lightbox, iframe_width, iframe_height) {
  if (lightbox) {
    vidyard_whLu9ete8y4sADcYxMK8iX_params.autoplay = true;
  }

  var referring_url = encodeURIComponent(document.referrer);
  if (referring_url) {
    vidyard_whLu9ete8y4sADcYxMK8iX_params.referring_url = referring_url;
  }
  ...
  if (document.documentMode === 7) {
    ...
    var ie7RepaintIframe = function() {
      var iframe = Vidyard.Helpers.getPlayerIFrame('whLu9ete8y4sADcYxMK8iX');
  ...
  if (vidyard_secure_whLu9ete8y4sADcYxMK8iX === true) {
    query_params_string = vidyard_whLu9ete8y4sADcYxMK8iX_raw_params;
  } else {
    for (var param in vidyard_whLu9ete8y4sADcYxMK8iX_params) {
      var param_value = vidyard_whLu9ete8y4sADcYxMK8iX_params[param];
      if (param_value === true) {
        param_value = '1';
      }
  ...
  return '<span id="vidyard_span_whLu9ete8y4sADcYxMK8iX" style="display: block; margin: auto; position: relative; width: ' + iframe_width + 'px; height: ' + iframe_height + 'px;"><iframe id="vidyard_iframe_whLu9ete8y4sADcYxMK8iX" class="vidyard_iframe" src="//play.vidyard.com/whLu9ete8y4sADcYxMK8iX?' + query_params_string + '" width="' + iframe_resizer + '" height="100%" title="Video" aria-label="Video" scrolling=no frameborder=0 allowtransparency="true" allowfullscreen allow="autoplay" style="opacity: 0; background-color: transparent; position: absolute; ' + ios_width_fix + ' right: 0px; top: 0px;"></iframe></span>';
}

// --- If the browser is FF, reload the iframe with the given id
function fn_vidyard_iframe_after_load_whLu9ete8y4sADcYxMK8iX() {
  var _theframe = Vidyard.Helpers.getPlayerIFrame('whLu9ete8y4sADcYxMK8iX');
  if (_theframe) {
    ...
    if (vidyard_secure_whLu9ete8y4sADcYxMK8iX === true && vidyard_whLu9ete8y4sADcYxMK8iX_params.type === 'lightbox') {
      vidyard_integration_check_whLu9ete8y4sADcYxMK8iX();
    }
    ...
    if (vidyard_whLu9ete8y4sADcYxMK8iX_params.marketo_lead) {
      var message = {'data': {'type': 'marketo', 'value': vidyard_whLu9ete8y4sADcYxMK8iX_params.marketo_lead }};
      message.event = 'identifyVisitor';
      message.uuid = 'whLu9ete8y4sADcYxMK8iX';
      Vidyard.Helpers.getPlayerIFrame('whLu9ete8y4sADcYxMK8iX')
...
function fn_vidyard_whLu9ete8y4sADcYxMK8iX() {
  var vidyard = {};
    ...
    return {
      show:function(initial_iframe_settings) {
        var showCore = function () {
          ...
          var width_height = this.lock_to_aspect_ratio(iframe_settings.width, iframe_settings.height, (319 + (vidyard_player_width_whLu9ete8y4sADcYxMK8iX / 5)));
      ...
      fill_iframe: function(settings_html_url, has_element, use_post_request, should_animate, width, height) {
        if(has_element) {
          if(iframe_settings.image) {
            ...
            this.set_tinner_size(fn_vidyard_build_iframe_whLu9ete8y4sADcYxMK8iX(true, width, height), should_animate, width, height);
          } else {
      ...
      resize: function() {
        ...
        var width_height = v_box.lock_to_aspect_ratio(
            vidyard_player_width_whLu9ete8y4sADcYxMK8iX,
            vidyard_player_height_whLu9ete8y4sADcYxMK8iX,
            319 + (vidyard_player_width_whLu9ete8y4sADcYxMK8iX / 5)
        );

        iframe_settings.width = width_height[0];
        iframe_settings.height = width_height[1];

        var vidyard_iframe = Vidyard.Helpers.getPlayerIFrame('whLu9ete8y4sADcYxMK8iX');
        var vidyard_span = document.getElementById('vidyard_span_whLu9ete8y4sADcYxMK8iX');
      ...
      fade_element: function(element_to_fade, fading_in, target_alpha) {
        ...
        if (old_opacity == target_alpha) {
          ...
          if (fading_in == -1) {
            ...
            Vidyard.Helpers.getPlayerIFrame('whLu9ete8y4sADcYxMK8iX').src = '';
  ...
  vidyard.box.show({
    iframe: '#', width: vidyard_player_width_whLu9ete8y4sADcYxMK8iX,
    height: vidyard_player_height_whLu9ete8y4sADcYxMK8iX,
    animate: !vidyard_html5_whLu9ete8y4sADcYxMK8iX,
    fixed: true
  });
}
