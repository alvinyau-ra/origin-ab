var vidyard_player_width_ePVjFstAXMXxvtKaWpB6CP = 640;
var vidyard_player_height_ePVjFstAXMXxvtKaWpB6CP = 360;
var vidyard_html5_ePVjFstAXMXxvtKaWpB6CP = false;
var vidyard_secure_ePVjFstAXMXxvtKaWpB6CP = false;
var vidyard_integration_check_ePVjFstAXMXxvtKaWpB6CP = function() {};
var vidyard_ePVjFstAXMXxvtKaWpB6CP_params = {};
var vidyard_ePVjFstAXMXxvtKaWpB6CP_raw_params = '';
...
(function() {
  ...
  var embed_script = document.getElementById('vidyard_embed_code_ePVjFstAXMXxvtKaWpB6CP');
  var playerDetails = {"keywords":"FLEX 5000 I/O,FLEX I/O,I/O 模組,Logix 5000 系列,分散式 I/O 解決方案,安全 I/O 模組,機櫃模組化","thumbnailUrl":"https://play.vidyard.com/ePVjFstAXMXxvtKaWpB6CP.jpg","@context":"http://schema.org/","@type":"VideoObject","id":"ePVjFstAXMXxvtKaWpB6CP","description":"FLEX 5000 I/O 支援多種應用,可透過與加強版 1 Gb EtherNet/IP 的連接以及具有彈性的安裝選項提供企業聯網解決方案，以智慧控制的方式更快速地回應流程。\r\n\r\n深入瞭解：https://ab.rockwellautomation.com/IO/In-Cabinet-Modular/FLEX-5000-IO-Modules","name":"FLEX 5000™ I/O 概要","transcript":"","uploadDate":"2018-08-01T03:06:31.000Z","duration":"T3M52S","embedUrl":"https://play.vidyard.com/ePVjFstAXMXxvtKaWpB6CP","height":360,"width":640};
  ...
  function check_for_duplicate_embeds() {
    ...
    for (var i = 0; i < all_scripts.length; ++i) {
      if (all_scripts[i].getAttribute('id') === "vidyard_embed_code_ePVjFstAXMXxvtKaWpB6CP") {
    ...
    if (dupe_scripts.length >= 1) {
      ...
      if (typeof document.querySelectorAll === 'function') {
        ...
        for (var i = 0; i < wrappers.length; ++i) {
          if (wrappers[i].getAttributeNode('onclick').value === "fn_vidyard_ePVjFstAXMXxvtKaWpB6CP();") {
  ...
  if (!embed_script || !embed_script.src) {
    ...
    for (var i in scripts) {
      ...
      if (script.src && /^.+(vidyard\.com).*\/ePVjFstAXMXxvtKaWpB6CP.*/.test(script.src)) {
        embed_script = script;
        break;
      }
    }
  ...
  if (embed_script && embed_script.src) {
    ...
    vidyard_ePVjFstAXMXxvtKaWpB6CP_raw_params = rawQueryParams;
    ...
    embed_script.src.replace(/(?:[\?&])([^&=]*)=?([^&#]*)/g, function ($0, $1, $2) {
      if ($1) {
        switch ($1) {
          ...
          case 'vylegacy':
            vidyard_ePVjFstAXMXxvtKaWpB6CP_params[$1] = (parseInt($2, 10) !== 0);
            break;
          case 'chapter':
            vidyard_ePVjFstAXMXxvtKaWpB6CP_params[$1] = parseInt($2, 10);
            break;
          case 'width':
          ...
          case 'height':
            ...
            break;
          case 'X-VY-Signature':
            vidyard_secure_ePVjFstAXMXxvtKaWpB6CP = true;
          default:
            vidyard_ePVjFstAXMXxvtKaWpB6CP_params[$1] = $2;
            break;
        }
      }
    });
  ...
  if (!vidyard_ePVjFstAXMXxvtKaWpB6CP_params.type || vidyard_ePVjFstAXMXxvtKaWpB6CP_params.type !== 'lightbox') {
    vidyard_ePVjFstAXMXxvtKaWpB6CP_params.type = 'inline';
  }

  // Determine Player Settings which effect embed
  var multi_chapter = false && !(typeof vidyard_ePVjFstAXMXxvtKaWpB6CP_params.hide_html5_playlist !== 'undefined' && vidyard_ePVjFstAXMXxvtKaWpB6CP_params.hide_html5_playlist);
  var playlist_always_open = false;
  if (typeof vidyard_ePVjFstAXMXxvtKaWpB6CP_params.playlist_always_open !== 'undefined') {
    playlist_always_open = vidyard_ePVjFstAXMXxvtKaWpB6CP_params.playlist_always_open;
  }
  ...
  if (query_player_width && query_player_width > 0 && query_player_height && query_player_height > 0) {
    vidyard_player_width_ePVjFstAXMXxvtKaWpB6CP = query_player_width;
    vidyard_player_height_ePVjFstAXMXxvtKaWpB6CP = query_player_height;
  } else if (query_player_width && query_player_width > 0) {
    query_player_height = (query_player_width * vidyard_player_height_ePVjFstAXMXxvtKaWpB6CP) / vidyard_player_width_ePVjFstAXMXxvtKaWpB6CP;
    vidyard_player_width_ePVjFstAXMXxvtKaWpB6CP = query_player_width;
    vidyard_player_height_ePVjFstAXMXxvtKaWpB6CP = query_player_height;
  } else if (query_player_height && query_player_height > 0) {
    query_player_width = (query_player_height * vidyard_player_width_ePVjFstAXMXxvtKaWpB6CP) / vidyard_player_height_ePVjFstAXMXxvtKaWpB6CP;
    vidyard_player_width_ePVjFstAXMXxvtKaWpB6CP = query_player_width;
    vidyard_player_height_ePVjFstAXMXxvtKaWpB6CP = query_player_height;
  }

  var iframe_container = document.getElementById("vidyard_ePVjFstAXMXxvtKaWpB6CP");
  ...
  if (!iframe_container && vidyard_ePVjFstAXMXxvtKaWpB6CP_params.type === 'inline') {
    iframe_container = document.createElement("span");
    iframe_container.id = 'vidyard_ePVjFstAXMXxvtKaWpB6CP';
  ...
  if (vidyard_secure_ePVjFstAXMXxvtKaWpB6CP === false) {
    // Get cookie data/ids from various MAPs/CRMs
    var pardot_id = fn_getCookie("pardot");
    if (pardot_id) {
      vidyard_ePVjFstAXMXxvtKaWpB6CP_params.pardot_id = pardot_id;
    }

    var hubspot_id = fn_getCookie("hubspotutk");
    if (hubspot_id) {
      vidyard_ePVjFstAXMXxvtKaWpB6CP_params.hubspot_id = hubspot_id;
    }
    ...
    if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
      vidyard_ePVjFstAXMXxvtKaWpB6CP_params.marketo_id = encodeURIComponent(marketo_id);
    }

    var dreamforce_id = fn_getCookie("vy_dreamforce");
    if (dreamforce_id) {
      vidyard_ePVjFstAXMXxvtKaWpB6CP_params.dreamforce_id = encodeURIComponent(dreamforce_id);
    }
    ...
    if (vycustom_id !== "") {
      vidyard_ePVjFstAXMXxvtKaWpB6CP_params.custom_id = vycustom_id;
    }

    // Gets the vyemail query string param, which connects views to an email entered
    var vyemail = get_parameter_by_name('vyemail');
    if (vyemail !== "") {
      vidyard_ePVjFstAXMXxvtKaWpB6CP_params.email = vyemail;
    }

    var vyac = get_parameter_by_name('vyac');
    var vyplayer = get_parameter_by_name('vyplayer');
    if (vyplayer === 'ePVjFstAXMXxvtKaWpB6CP') {
      vidyard_ePVjFstAXMXxvtKaWpB6CP_params.access_code = vyac;
    }
  }
  ...
  if (navigator.userAgent.toLowerCase().match(/ip(ad|od|hone)|android/)) {
    vidyard_html5_ePVjFstAXMXxvtKaWpB6CP = true;
    vidyard_ePVjFstAXMXxvtKaWpB6CP_params.disable_popouts = true;
  }
  ...
  function create_embed() {
    ...
    var integration_tracking_check = function() {
      if (playerReady) {
        ...
        window.addEventListener('message', function callback(e) {
          ...
          if (data.event === "ready" && data.uuid === "ePVjFstAXMXxvtKaWpB6CP") {
    ...
    var integration_tracking_check_original = function () {
      ...
      if (!referringUrlSent && vidyard_secure_ePVjFstAXMXxvtKaWpB6CP === true && vidyard_ePVjFstAXMXxvtKaWpB6CP_params.referring_url) {
        var playerFrame = Vidyard.Helpers.getPlayerIFrame('ePVjFstAXMXxvtKaWpB6CP');
        var message = {
          'event': 'parentReferer',
          'uuid': 'ePVjFstAXMXxvtKaWpB6CP',
          'data': {
            'value': vidyard_ePVjFstAXMXxvtKaWpB6CP_params.referring_url
          }
        }
      ...
      if (!vidyard_ePVjFstAXMXxvtKaWpB6CP_params.pardot_id) {
        var pardot_id = fn_getCookie("pardot");
        if (pardot_id) {
          vidyard_ePVjFstAXMXxvtKaWpB6CP_params.pardot_id = pardot_id;
      ...
      if (!vidyard_ePVjFstAXMXxvtKaWpB6CP_params.hubspot_id) {
        var hubspot_id = fn_getCookie("hubspotutk");
        if (hubspot_id) {
          vidyard_ePVjFstAXMXxvtKaWpB6CP_params.hubspot_id = hubspot_id;
      ...
      if (!vidyard_ePVjFstAXMXxvtKaWpB6CP_params.dreamforce_id) {
        var dreamforce_id = fn_getCookie("vy_dreamforce");
        if (dreamforce_id) {
          dreamforce_id = encodeURIComponent(dreamforce_id);
          vidyard_ePVjFstAXMXxvtKaWpB6CP_params.dreamforce_id = dreamforce_id;
          messages.push({'data': {'type': 'dreamforce', 'value': dreamforce_id}});
        }
      }
      if (!vidyard_ePVjFstAXMXxvtKaWpB6CP_params.marketo_id) {
        var marketo_id = fn_getCookie("_mkto_trk");
        var munchkin_id = '';
        
        if (marketo_id && munchkin_id && marketo_id.toLowerCase().indexOf(munchkin_id) > -1) {
          marketo_id = encodeURIComponent(marketo_id);
          vidyard_ePVjFstAXMXxvtKaWpB6CP_params.marketo_id = marketo_id;
      ...
      if (vidyard_ePVjFstAXMXxvtKaWpB6CP_params.type === "inline" || vidyard_secure_ePVjFstAXMXxvtKaWpB6CP === true) {
        for(var i in messages) {
          messages[i].event = 'associateVisitor';
          messages[i].uuid = 'ePVjFstAXMXxvtKaWpB6CP';
          Vidyard.Helpers.getPlayerIFrame('ePVjFstAXMXxvtKaWpB6CP').contentWindow.postMessage(JSON.stringify(messages[i]), window.location.protocol + '//play.vidyard.com');
        }
      }
    ...
    vidyard_integration_check_ePVjFstAXMXxvtKaWpB6CP = integration_tracking_check;

    // Display the inline or lightbox
    if (vidyard_ePVjFstAXMXxvtKaWpB6CP_params.type === "inline") {
      // Load the iframe
      iframe_container.innerHTML = fn_vidyard_build_iframe_ePVjFstAXMXxvtKaWpB6CP(false, vidyard_player_width_ePVjFstAXMXxvtKaWpB6CP, vidyard_player_height_ePVjFstAXMXxvtKaWpB6CP);

      Vidyard.Helpers.addListener(
        'load',
        'onload',
        function() {
          fn_vidyard_iframe_after_load_ePVjFstAXMXxvtKaWpB6CP();
          integration_tracking_check();
        },
        Vidyard.Helpers.getPlayerIFrame('ePVjFstAXMXxvtKaWpB6CP')
      );

    } else if (vidyard_ePVjFstAXMXxvtKaWpB6CP_params.type === "lightbox") {
      ...
      if (document.readyState === 'complete') {
        if (!vidyard_secure_ePVjFstAXMXxvtKaWpB6CP) {
          integration_tracking_check();
        }
  ...
  Vidyard._players['ePVjFstAXMXxvtKaWpB6CP'] = '';
}());
...
var vyContactCallback = function(data) {
  ...
  vidyard_ePVjFstAXMXxvtKaWpB6CP_params.marketo_lead = marketoData;
  ...
  // or tried again after IFrame loads in fn_vidyard_iframe_after_load_ePVjFstAXMXxvtKaWpB6CP
  playerIframe = Vidyard.Helpers.getPlayerIFrame('ePVjFstAXMXxvtKaWpB6CP');
  if ( playerIframe ) {
    message = {'data': {'type': 'marketo', 'value': marketoData}};
    message.event = 'identifyVisitor';
    message.uuid = 'ePVjFstAXMXxvtKaWpB6CP';
...
(function () {
  function async_load() {
    var contactId = vidyard_ePVjFstAXMXxvtKaWpB6CP_params.marketo_id;
    if (!contactId) {
    ...
    var url = window.location.protocol + '//play.vidyard.com'
        + '/v3/ePVjFstAXMXxvtKaWpB6CP/contact/' + encodeURIComponent(contactId) + '/marketoContact.js';
...
function fn_vidyard_build_iframe_ePVjFstAXMXxvtKaWpB6CP(lightbox, iframe_width, iframe_height) {
  if (lightbox) {
    vidyard_ePVjFstAXMXxvtKaWpB6CP_params.autoplay = true;
  }

  var referring_url = encodeURIComponent(document.referrer);
  if (referring_url) {
    vidyard_ePVjFstAXMXxvtKaWpB6CP_params.referring_url = referring_url;
  }
  ...
  if (document.documentMode === 7) {
    ...
    var ie7RepaintIframe = function() {
      var iframe = Vidyard.Helpers.getPlayerIFrame('ePVjFstAXMXxvtKaWpB6CP');
  ...
  if (vidyard_secure_ePVjFstAXMXxvtKaWpB6CP === true) {
    query_params_string = vidyard_ePVjFstAXMXxvtKaWpB6CP_raw_params;
  } else {
    for (var param in vidyard_ePVjFstAXMXxvtKaWpB6CP_params) {
      var param_value = vidyard_ePVjFstAXMXxvtKaWpB6CP_params[param];
      if (param_value === true) {
        param_value = '1';
      }
  ...
  return '<span id="vidyard_span_ePVjFstAXMXxvtKaWpB6CP" style="display: block; margin: auto; position: relative; width: ' + iframe_width + 'px; height: ' + iframe_height + 'px;"><iframe id="vidyard_iframe_ePVjFstAXMXxvtKaWpB6CP" class="vidyard_iframe" src="//play.vidyard.com/ePVjFstAXMXxvtKaWpB6CP?' + query_params_string + '" width="' + iframe_resizer + '" height="100%" title="Video" aria-label="Video" scrolling=no frameborder=0 allowtransparency="true" allowfullscreen allow="autoplay" style="opacity: 0; background-color: transparent; position: absolute; ' + ios_width_fix + ' right: 0px; top: 0px;"></iframe></span>';
}

// --- If the browser is FF, reload the iframe with the given id
function fn_vidyard_iframe_after_load_ePVjFstAXMXxvtKaWpB6CP() {
  var _theframe = Vidyard.Helpers.getPlayerIFrame('ePVjFstAXMXxvtKaWpB6CP');
  if (_theframe) {
    ...
    if (vidyard_secure_ePVjFstAXMXxvtKaWpB6CP === true && vidyard_ePVjFstAXMXxvtKaWpB6CP_params.type === 'lightbox') {
      vidyard_integration_check_ePVjFstAXMXxvtKaWpB6CP();
    }
    ...
    if (vidyard_ePVjFstAXMXxvtKaWpB6CP_params.marketo_lead) {
      var message = {'data': {'type': 'marketo', 'value': vidyard_ePVjFstAXMXxvtKaWpB6CP_params.marketo_lead }};
      message.event = 'identifyVisitor';
      message.uuid = 'ePVjFstAXMXxvtKaWpB6CP';
      Vidyard.Helpers.getPlayerIFrame('ePVjFstAXMXxvtKaWpB6CP')
...
function fn_vidyard_ePVjFstAXMXxvtKaWpB6CP() {
  var vidyard = {};
    ...
    return {
      show:function(initial_iframe_settings) {
        var showCore = function () {
          ...
          var width_height = this.lock_to_aspect_ratio(iframe_settings.width, iframe_settings.height, (319 + (vidyard_player_width_ePVjFstAXMXxvtKaWpB6CP / 5)));
      ...
      fill_iframe: function(settings_html_url, has_element, use_post_request, should_animate, width, height) {
        if(has_element) {
          if(iframe_settings.image) {
            ...
            this.set_tinner_size(fn_vidyard_build_iframe_ePVjFstAXMXxvtKaWpB6CP(true, width, height), should_animate, width, height);
          } else {
      ...
      resize: function() {
        ...
        var width_height = v_box.lock_to_aspect_ratio(
            vidyard_player_width_ePVjFstAXMXxvtKaWpB6CP,
            vidyard_player_height_ePVjFstAXMXxvtKaWpB6CP,
            319 + (vidyard_player_width_ePVjFstAXMXxvtKaWpB6CP / 5)
        );

        iframe_settings.width = width_height[0];
        iframe_settings.height = width_height[1];

        var vidyard_iframe = Vidyard.Helpers.getPlayerIFrame('ePVjFstAXMXxvtKaWpB6CP');
        var vidyard_span = document.getElementById('vidyard_span_ePVjFstAXMXxvtKaWpB6CP');
      ...
      fade_element: function(element_to_fade, fading_in, target_alpha) {
        ...
        if (old_opacity == target_alpha) {
          ...
          if (fading_in == -1) {
            ...
            Vidyard.Helpers.getPlayerIFrame('ePVjFstAXMXxvtKaWpB6CP').src = '';
  ...
  vidyard.box.show({
    iframe: '#', width: vidyard_player_width_ePVjFstAXMXxvtKaWpB6CP,
    height: vidyard_player_height_ePVjFstAXMXxvtKaWpB6CP,
    animate: !vidyard_html5_ePVjFstAXMXxvtKaWpB6CP,
    fixed: true
  });
}
