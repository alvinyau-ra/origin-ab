...
function getLocalizedUrl(sUrl, sLocale) {
       // This array contains a set of values that are not actually locales, yet
       // can be recognized as locale strings by the regular expression matcher
       // Values in this list will be handled properly
       // NOTE: The leading '/' is important as the regular expression matches against that
       var aNonLocaleValues = [ '/IO', ];
       
       // This implementation conforms to the ECMA-262 specification from:
       // https://developer.mozilla.org/en-US/docs/JavaScript/Reference/Global_Objects/Array/indexOf
    var fIndexOf = function (array, searchElement) {
        "use strict";
        if (array == null) {
            throw new TypeError();
        }
        ...
        for (; k < len; k++) {
            if (k in t && t[k].toUpperCase() === searchElement.toUpperCase()) {
                return k;
            }
        }
        return -1;
    ...
    // Match an replace locale for any /allenbradley_xx/ URL (localized page)
    var oMatch = /allenbradley(_\w\w)?/.exec (sUrl);
    if (oMatch != null) {
        if (sLocale != '') {
            return sUrl.replace(/allenbradley(_\w\w)?/,
                                'allenbradley_' + sLocale);
        } else {
            return sUrl.replace(/allenbradley(_\w\w)?/,
                                'allenbradley');
        }
    ...
    // Match and replace locale for any /xx/ URL (localized page)
    oMatch = /\/\w\w\//.exec(sUrl);
    if (oMatch != null) {
       var sMatch = oMatch[0];
       sMatch = sMatch.substr(0, sMatch.length - 1);
       if (fIndexOf (aNonLocaleValues, sMatch) != -1) {
              if (sLocale != '') {
               return sUrl.replace(/\/\w\w\//, '/' + sLocale + oMatch);
              }
              return sUrl.replace(/\/\w\w\//, oMatch);
       }
       
       if (sLocale != '') {
               return sUrl.replace(/\/\w\w\//, '/' + sLocale +  '/');
       }
       return sUrl.replace(/\/\w\w\//, '/');
    }
   
    // Match and replace locale for any /xx at the end of the URL (locale index page)
    oMatch = /\/\w\w$/.exec(sUrl);
    if (oMatch != null) {
       // Check if the match is not actually a locale and append that to the URL
       if (fIndexOf(aNonLocaleValues, oMatch[0]) != -1) {
              if (sLocale != '') {
                   return sUrl.replace(/\/\w\w$/, '/' + sLocale + oMatch);
              }
            return sUrl.replace(/\/\w\w$/, oMatch);
    ...
    // Append the locale code for the com/ URL (default index page)
    if (sLocale != '') {
        return sUrl.replace (/com\//, 'com/' + sLocale + '/');
    }
    
    // Return the original url as it did not match any of our rules
    return sUrl;
}
